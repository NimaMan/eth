...
use super::model::{
    ...
    ProtocolBucket, RunSummary,
};

pub(super) fn issue_flags(
    summary: &RunSummary,
...
pub(super) async fn load_summary(pool: &PgPool, run_id: &str) -> Result<RunSummary> {
    let row = sqlx::query(
        r#"
        WITH latest AS (
...
pub(super) async fn load_concentration(pool: &PgPool, run_id: &str) -> Result<PnlConcentration> {
    let row = sqlx::query(
        r#"
        WITH latest AS (
            SELECT DISTINCT ON (ps.run_id, ps.position_id)
...
pub(super) async fn load_failures(pool: &PgPool, run_id: &str) -> Result<Vec<FailureBucket>> {
    let rows = sqlx::query(
        r#"
        SELECT
            CASE
...
pub(super) async fn load_buy_failed_entries(
    pool: &PgPool,
    run_id: &str,
...
pub(super) async fn load_open_failed_exits(
    pool: &PgPool,
    run_id: &str,
    limit: i64,
...
pub(super) async fn load_protocols(
    pool: &PgPool,
    run_id: &str,
    replay_run_id: Option<&str>,
...
pub(super) async fn load_ranked_positions(
    pool: &PgPool,
    run_id: &str,
    limit: i64,
    direction: &str,
