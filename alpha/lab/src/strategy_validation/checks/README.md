...
| `live_chain_sim_uses_chain_server_simulator` | Does live chain-sim use the chain-server LiveTxSimulator? | Live chain-sim should not depend on an Alpha-local simulator; result-set and run metadata must show the chain-server source. | `metadata.rs` |
| `live_block_frame_runtime_metadata` | Does live chain-sim use block-frame inputs? | Live chain-sim decisions should be pinned to pushed block frames rather than late pulls from latest token/pool state. | `metadata.rs` |
...
| `historical_mempool_observations` | Did a historical backtest avoid mempool strategy observations? | Historical backtests must not process live mempool signals through strategy observation rows. | `signal_scope.rs` |
...
| `mempool_risk_uses_detector_head_block` | Do mempool risk events use detector-time block evidence? | Pending mempool signals must carry their own detection head; Alpha must not stamp them with the later block frame it is consuming. | `signal_scope.rs` |
...
| `chain_sim_trading_enabled_mempool_skip` | Does chain-sim explicitly skip trading-enabled mempool entries? | Chain-sim may prime first-poll `trading_enabled` mempool signals, but processed ones must be ignored with the documented pool-update entry-path reason. | `signal_scope.rs` |
| `no_settlement_wait_mempool_deferrals` | Has the old local settlement-state deferral path been removed? | After delegating simulation to chain-server, Alpha should not defer mempool observations while waiting for local settlement state. | `signal_scope.rs` |
...
| `chain_sim_has_no_mempool_tail_entry_orders` | Did chain-sim avoid mempool tail-entry orders? | Current chain-sim live backtests validate mined pool-update entries; same-block mempool tail-entry submission is a live-real concern. | `execution_replay.rs` |
...
| `tail_entry_buy_priority_undercuts_dependency` | Do tail-entry buys undercut the dependency priority fee? | The persisted gas-policy evidence should show the selected priority fee is below the enabling transaction by the configured undercut. | `execution_replay.rs` |
