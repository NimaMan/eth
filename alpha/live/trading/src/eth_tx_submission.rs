...
pub struct EthTxExecutorSubmitClientConfig {
    pub base_url: String,
    pub bearer_token: String,
}

impl EthTxExecutorSubmitClientConfig {
...
#[derive(Clone)]
pub struct EthTxExecutorSubmitClient {
    http: reqwest::Client,
    config: EthTxExecutorSubmitClientConfig,
}

impl EthTxExecutorSubmitClient {
    pub fn new(config: EthTxExecutorSubmitClientConfig) -> Self {
        Self {
    ...
    ) -> Result<EthTxSubmitTransactionResult, EthTxExecutorSubmitClientError> {
        self.submit_transaction(&EthTxSubmitTransactionRequest {
    ...
    pub async fn submit_direct_raw(
    ...
    ) -> Result<EthTxSubmitDirectRawResult, EthTxExecutorSubmitClientError> {
        if self.config.bearer_token.trim().is_empty() {
            return Err(EthTxExecutorSubmitClientError::Config(
        ...
        if !status.is_success() {
            ...
            return Err(EthTxExecutorSubmitClientError::Server {
        ...
        response
            .json::<EthTxSubmitDirectRawResult>()
            .await
            .map_err(EthTxExecutorSubmitClientError::Http)
    }
    ...
    pub async fn submit_transaction(
        ...
        response
            .json::<EthTxSubmitTransactionResult>()
            .await
            .map_err(EthTxExecutorSubmitClientError::Http)
    }
}
...
pub enum EthTxExecutorSubmitClientError {
    #[error("configuration error: {0}")]
    Config(String),
    #[error("ETH tx executor HTTP error: {0}")]
    Http(#[from] reqwest::Error),
    #[error("ETH tx executor rejected transaction request with HTTP {status}: {body}")]
...
pub struct LiveDirectRawTransactionRequest {
    ...
    pub bribe: Option<EthTxBribeRequest>,
    pub simulation: Option<EthTxSimulationReference>,
    #[serde(default = "default_metadata")]
    pub metadata: Value,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct EthTxBribeRequest {
    pub priority_fee_per_gas: String,
    pub max_fee_per_gas: Option<String>,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct EthTxSimulationReference {
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct EthTxSubmitDirectRawResult {
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct EthTxSubmitTransactionRequest {
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct EthTxSubmitTransactionResult {
    pub attempt_id: String,
    pub status: String,
    pub tx_hash: Option<String>,
    #[serde(default)]
...
impl From<EthTxSubmitDirectRawResult> for EthTxSubmitTransactionResult {
...
mod tests {
    ...
    fn direct_raw_endpoint_is_joined_without_double_slash() {
        let config = EthTxExecutorSubmitClientConfig::new("http://127.0.0.1:5006/", "token");

        assert_eq!(
            config.direct_raw_url(),
            "http://127.0.0.1:5006/eth/tx/direct-raw"
        );
    }
    ...
    #[test]
    fn public_submission_policy_serializes_as_eth_tx_policy() {
        let request = EthTxSubmitTransactionRequest {
            transaction: request(),
            submission_policy: TxSubmissionPolicy::PublicRpcBroadcast,
        };
    ...
    #[test]
    fn serde_defaults_match_eth_tx_executor_direct_raw_contract() {
