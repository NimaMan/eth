...
3. Confirm the ETH tx executor is healthy and in `dry_run`.
...
9. Switch only ETH tx executor broadcast mode to `broadcast`.
...
15. Return the ETH tx executor to `dry_run` unless the next approved step
    explicitly needs public broadcast.
