...
use crate::core::rules as shared_rules;

use super::{
    config::StrategyConfig,
    rule::RuleDecision,
    rules::entry,
    state::{EngineState, RestoredEntryBankroll},
};
...
impl StrategyEngine {
    ...
    pub fn with_bought_pools(
        ...
        Self {
            config,
            state: EngineState::with_bought_pools(bought_pools),
        }
    }
    ...
    pub fn with_restored_state(
        ...
        Self {
            config,
            state: EngineState::with_bought_pools_and_active_hold_blocks(
                bought_pools,
                active_hold_blocks,
            ),
        }
    ...
    fn evaluate_entry_for_pool(
        ...
        Ok(match entry::buy_once::evaluate(&self.state, pool) {
...
fn position_entry_spend_wei(position: &Position, config: &StrategyConfig) -> U256 {
    position
        .entry_cost_basis
