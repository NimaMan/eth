...
use eth_alpha_core::{
    Amount, ExecutionReport, ExecutionStatus, OrderId, OrderSide, PoolProtocol, PortfolioId,
    Position, PositionId, PositionKey, PositionState, StrategyName, TokenPoolId, WalletId,
};
...
#[test]
fn deferred_buy_can_retry_same_position() {
    let mut position = position();
    let first_order_id = OrderId("buy-1".to_string());
    let second_order_id = OrderId("buy-2".to_string());

    position.mark_intent_created(OrderSide::Buy).unwrap();
    position
        .mark_order_submitted(first_order_id.clone(), OrderSide::Buy)
        .unwrap();
    position
        .apply_execution_report(&ExecutionReport {
    ...
    position.mark_intent_created(OrderSide::Buy).unwrap();
    position
        .mark_order_submitted(second_order_id.clone(), OrderSide::Buy)
        .unwrap();

    assert_eq!(position.state, PositionState::BuySubmitted);
    assert_eq!(position.entry_order_id.as_ref(), Some(&second_order_id));
    assert!(!position.has_exposure());
}
...
#[test]
fn uneconomic_sell_cancellation_keeps_exposure_but_blocks_retry() {
    let mut position = position();
    let buy_order_id = OrderId("buy-1".to_string());
    let sell_order_id = OrderId("sell-1".to_string());
    ...
    assert_eq!(position.state, PositionState::SellCancelled);
    assert!(position.has_exposure());
    assert!(!position.can_submit_exit());
    assert_eq!(
        position.exit_failure_reason.as_deref(),
        Some("uneconomic sell: simulated WETH proceeds 1 wei <= gas cost 2 wei")
    );
    assert!(position.mark_intent_created(OrderSide::Sell).is_err());
}
