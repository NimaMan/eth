...
5. Submit `DirectRawTransactionRequest` to `tx_executor::EthTxExecutor`.
6. Poll for receipt and map real `tx_hash` back to `TokenPoolId`.
7. Return `ExecutionReport` with on-chain fill results.
...
slippage, or AMM routes. This adapter is the **only** component in `alpha/engine`
that talks to `tx_executor`.
