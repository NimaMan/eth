...
const DEFAULT_REPORT_DIR: &str =
    "/home/nima/code/crypto/blockchains/eth/alpha/lab/reports/eth_tx_calibration";
const DEFAULT_PLANNER_FIXTURE_STRATEGY: &str =
    "snipe-all-risk-atlas-lp-gate-hold15-buy-confirm-lp-maxhold";
const DEFAULT_PLANNER_FIXTURE_RUN_ID: &str = "eth-tx-calibration-planner-fixture";
...
struct Args {
    ...
    /// contacting ETH tx executor. Requires --planner-fixture.
    #[arg(long, default_value_t = false)]
    ...
    /// Env var used for ETH tx executor auth.
    ...
    /// Allow submitting calibration requests when ETH tx executor is not in dry_run.
    ...
    /// Replace each request simulation.block_number with ETH tx executor's current RPC
...
async fn main() -> Result<()> {
    ...
    let config = CalibrationRunConfig {
        eth_tx_executor_base_url: args.eth_tx_executor_url.clone(),
        bearer_token: token,
    ...
    let report = run_calibration(config, input, args.expect.into())
        .await
        .wrap_err("ETH tx executor calibration failed before report generation")?;
    ...
    if args.print_json {
        ...
        println!(
            "ETH tx executor calibration {:?}: {}/{} passed, {} failed, {} unsafe",
    ...
    match report.summary.verdict {
        ...
        CalibrationOverallVerdict::Failed => Err(eyre!("ETH tx executor calibration failed")),
        CalibrationOverallVerdict::Unsafe => {
            Err(eyre!("ETH tx executor calibration was unsafe to submit"))
        }
    }
}
...
fn load_token(primary_env: &str) -> Result<String> {
    ...
    Err(eyre!(
        "missing ETH tx executor bearer token; set {}",
        primary_env
    ))
}
...
fn write_report(args: &Args, report: &eth_live_trading::CalibrationReport) -> Result<PathBuf> {
    let path = match &args.report_path {
        ...
        None => {
            ...
            args.report_dir
                .join(format!("eth-tx-calibration-{timestamp}.json"))
        }
    };
