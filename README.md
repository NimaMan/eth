...
| Where are current pipeline bottlenecks tracked? | `Plan.md` | service memory, cache fill/read metrics, live readiness, mempool timing, alpha decision bottlenecks |
...
| Token PnL store | PostgreSQL schema `token_pnl`, configured by `databases.token_pnl.url`. | `eth_token_store/README.md` | Pool-scoped address PnL ledger and rollups: calculation runs, pool conservation totals, address-level PnL, and movement rows. |
| Token state store | PostgreSQL schema `token_state`, configured by `databases.token_state.url`. | `eth_token_store/README.md` | Latest token and token-pool read model: activity blocks, reserves, lifecycle, trading flags, valuation status, and liquidity-removal state. Historical runs write isolated scopes instead of overwriting live state. |
