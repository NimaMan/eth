...
async fn main() -> Result<()> {
    ...
    println!("persist_moves:   {}", args.persist_movements);
    ...
    let store = if args.dry_run {
        ...
        run.metadata = json!({
            ...
            "aggregate_only": !args.persist_movements,
            "movement_rows_persisted": args.persist_movements,
    ...
    fn parse() -> Result<Self> {
        ...
        while let Some(arg) = raw.next() {
            match arg.as_str() {
                ...
                "--persist-movements" => args.persist_movements = true,
                "--help" | "-h" => {
                    print_usage();
                    std::process::exit(0);
                }
...
async fn flush_pool_requests(
    ...
    for request in requests {
        ...
        if let Some(store) = store {
            if persist_movements {
                store.write_pool_export(run_id, &export).await?;
            } else {
...
Options:
  ...
  --persist-movements        Persist auditable movement rows and exact address tx counts
"
    );
}
