...
Agents should follow the execution-mode matrix in the top-level
`risk_atlas/README.md`. Large modelling runs are headless by default.
Server-backed runs are only the default when the user asks for frontend
range-builder inspection.
...
Risk Atlas writes PostgreSQL tables named `risk_atlas_*` in the database
configured by `databases.risk_atlas.url` in `blockchains/eth/config.toml`.
`eth_chain_server`, range exports, headless generation, and the standalone
`eth_risk_atlas` CLI all use that Risk Atlas database key.
...
risk_atlas/src/
  ...
  db/         # schema structs, reader, writer, migration runner
  ingest/     # source layout/contracts for current artifacts and future rows
  config.rs
  lib.rs
  main.rs     # small schema/import CLI
```
...
Apply migrations to the configured Risk Atlas Postgres database:
