...
| 2 | P1 | `mechanism_classification` | investigating | [SSS creator transferFrom pair drain](sss_creator_transfer_from_pair_drain_25041123/) | This is the second unresolved issue after Compass: exact chain truth now shows a creator `transferFrom(pair, drain_wallet, ...)` at tx `197`, creator `sync()` at tx `198`, and WETH drain sell at tx `200`; Alpha-route parity at those substates is still open. | Replay Alpha's actual executable sell route at pre-tx-197, post-tx-197, post-tx-198, and post-tx-200 coordinates, then decide whether this becomes a shared live mempool detector. |
...
| `mechanism_classification` | investigating | `sss_creator_transfer_from_pair_drain_25041123` | Focused SSS mechanism note: creator/control `transferFrom(pair, ...)` is the first state-changing warning, `sync()` publishes the manipulated balance, and the WETH drain follows in the same block. |
