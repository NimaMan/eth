...
| 2 | P2 | `display_read_model` | investigating | SSS reserve-quality detector/display | Chain/token-builder parity is reproduced; the remaining risk is misleading display or missing detector semantics for reserve-dust price explosions. | Read model suppresses displayed price/init when token reserve quality is unsafe, and Risk Atlas persists reserve-quality fields; next decide whether direct `sync()` should produce an earlier warning detector. |
...
| `display_read_model` | investigating | `sss_space_services_25041048` | Chain/token-builder parity is reproduced; display price/init is suppressed for token-reserve-dust pools, and durable observation/model fields are in place. |
