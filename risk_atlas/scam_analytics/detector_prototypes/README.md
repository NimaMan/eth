...
Detector prototypes for scam analytics start here before promotion into
production code. This folder is scoped to scam-analysis workflows; Risk Atlas
does not keep a generic top-level tools folder.
...
```text
risk_atlas/scam_analytics/detector_prototypes/range_triage.py \
...
The code lives under this folder because these rules are scam-analysis
prototypes, not production policy. Once a detector is proven by investigations
and regression coverage, promote the stable part into Rust crate logic,
`eth_chain_server`, `mempool_processor`, or the relevant owner module.

## Pool Classification

Scam-analysis severity uses the same pool classification contract as the strategy
...
parity checks, feed pool JSON into:
...
have reserves while the LP mint happened before the observed window; Risk Atlas
triage should report that as unknown LP supply, not as a zero-supply pool.
...
`risk_atlas/investigations/README.md`, but the prototype should not edit that
file by default.
