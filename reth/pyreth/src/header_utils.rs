...
fn parse_string_to_u64(value: &str) -> Result<u64> {
    ...
    if trimmed.starts_with("0x") || trimmed.starts_with("0X") {
        ...
        .map_err(|e| eyre!("failed to parse {trimmed} as hex u64: {e}"))
    } else {
        trimmed
            .parse::<u64>()
            .map_err(|e| eyre!("failed to parse {trimmed} as u64: {e}"))
    }
}
...
fn parse_string_to_u256(value: &str) -> Result<AlloyU256> {
    let trimmed = value.trim();
    if trimmed.starts_with("0x") || trimmed.starts_with("0X") {
        let bytes = hex::decode(trimmed.trim_start_matches("0x").trim_start_matches("0X"))?;
        Ok(AlloyU256::from_be_slice(&bytes))
    } else {
        AlloyU256::from_str(trimmed)
            .map_err(|e| eyre!("failed to parse {trimmed} as decimal u256: {e}"))
    }
}
    ...
    match value {
        ...
        Value::Array(arr) => {
            ...
            for v in arr {
                let byte = v
                    .as_u64()
                    .ok_or_else(|| eyre!("array byte values must be integers"))?;
                if byte > 0xFF {
                    return Err(eyre!("array byte value exceeds u8 range"));
                }
                bytes.push(byte as u8);
            }
            Ok(Some(bytes))
        }
        _ => Err(eyre!("unsupported value type for byte conversion")),
    }
}
