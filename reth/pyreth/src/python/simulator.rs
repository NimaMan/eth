...
impl PySimulator {
    ...
    fn __repr__(&self) -> String {
        format!("Simulator(backend='tx_simulator', version='{}')", 
                env!("CARGO_PKG_VERSION"))
    }
}
...
fn dict_to_unsigned_transaction(tx_dict: &PyDict) -> PyResult<UnsignedTransaction> {
    ...
    let gas = tx_dict.get_item("gas")?
        .map(|v| {
            if let Ok(s) = v.extract::<String>() {
                s.parse::<u64>()
                    .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("Invalid gas: {}", e)))
            } else if let Ok(n) = v.extract::<u64>() {
                Ok(n)
            } else {
                Err(PyErr::new::<pyo3::exceptions::PyValueError, _>("Gas must be string or int"))
            }
        })
        .transpose()?;
    ...
    let gas_price = tx_dict.get_item("gas_price")?
        .map(|v| {
            if let Ok(s) = v.extract::<String>() {
                s.parse::<u128>()
                    .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("Invalid gas_price: {}", e)))
            } else if let Ok(n) = v.extract::<u128>() {
                Ok(n)
            } else {
                Err(PyErr::new::<pyo3::exceptions::PyValueError, _>("Gas price must be string or int"))
            }
        })
        .transpose()?;
