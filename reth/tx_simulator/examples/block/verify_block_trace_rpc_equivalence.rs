...
use alloy_rpc_types_trace::geth::{GethDebugTracingOptions, TraceResult, GethTrace, CallConfig};
...
async fn main() -> Result<()> {
    ...
    let block = provider.block_by_number(block_to_verify)?
    ...
    let tracer_config = GethDebugTracingOptions {
        tracer: Some(alloy_rpc_types_trace::geth::GethDebugTracerType::BuiltInTracer(
            alloy_rpc_types_trace::geth::GethDebugBuiltInTracerType::CallTracer
        )),
        ..Default::default()
    };
    
    let our_traces_result = block_tracer.trace_block_by_number(block_to_verify, Some(tracer_config)).await;
    ...
    let rpc_response: Vec<Value> = match client.request(
    ...
    for (idx, (our_trace, rpc_trace)) in 
        our_traces.iter().zip(rpc_response.iter()).enumerate() 
    {
        ...
        match our_trace {
            TraceResult::Success { result: our_geth_trace, .. } => {
                ...
                if let Some(rpc_gas) = rpc_result["gasUsed"].as_str() {
                    let rpc_gas_val = u64::from_str_radix(rpc_gas.trim_start_matches("0x"), 16)
                        .unwrap_or(0);
                ...
                if let Some(rpc_value) = rpc_result["value"].as_str() {
                    if let Some(our_value) = &our_frame.value {
                        let rpc_value_int = u128::from_str_radix(
                            rpc_value.trim_start_matches("0x"), 
                            16
                        ).unwrap_or(0);
    ...
    println!("  • ❌ Mismatches: {} ({:.2}%)", mismatches, 100.0 - match_percentage);
    ...
    println!("  • Speedup: {:.1}x faster", rpc_time.as_secs_f64() / our_time.as_secs_f64());
