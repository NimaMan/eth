...
async fn main() -> Result<()> {
    ...
    let block = provider.block_by_number(block_to_trace)?
    ...
    // Get block header
    let header = provider.header_by_number(block_to_trace)?
    ...
    for (idx, tx_signed) in block.body.transactions.iter().enumerate() {
        ...
        match simulator.simulate_signed_transaction_with_trace_at_block(
            tx_signed, 
    ...
    println!("  • Average per tx: {:.2}ms", total_time.as_millis() as f64 / tx_count as f64);
    ...
    println!("  • Speedup: {:.1}x faster", rpc_estimate / total_time.as_secs_f64());
