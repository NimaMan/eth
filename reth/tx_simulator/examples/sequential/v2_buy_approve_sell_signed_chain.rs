//! Signed simulation chain: V2 buy (ETH->USDC on Uniswap V2), approve MAX (Sushi), sell (USDC->ETH on Sushi V2)
//! Uses only tx_simulator + reth_chain_query + ethers LocalWallet (no broadcast).

use tx_simulator::{TxSimulator, SignedTxChainSimulation};
use reth_chain_query::tx_builders::{
    amm_swap_route::AmmSwapRoute,
    build_buy_swap_with_min_out,
    build_sell_swap_with_min_out,
    build_approve_for_route,
};
...
async fn main() -> eyre::Result<()> {
    ...
    // Reth DB path
    let reth_db = env::var("RETH_DB_PATH").unwrap_or_else(|_| format!(
        "{}/.local/share/reth/mainnet",
        std::env::var("HOME").unwrap_or_else(|_|"/home/nima".into())
    ));
    let simulator = TxSimulator::new(&reth_db)?;
    println!("Reth DB: {} | latest {}", reth_db, simulator.get_latest_block()?);

    // Wallet from KARTAL_KILIT (dev) or exit
    let pk = env::var("KARTAL_KILIT").expect("Set KARTAL_KILIT to a dev private key");
    let chain_id: u64 = env::var("ETH_KARTAL_CHAIN_ID").ok().and_then(|v| v.parse().ok()).unwrap_or(1);
    ...
    let sushi_v2_usdc_weth: Address = alloy_to_ethers_addr(compute_sushiswap_pool(
        ethers_to_alloy_addr(weth), ethers_to_alloy_addr(usdc)
    ));
    let uni_route = AmmSwapRoute::UniswapV2 { pool: ethers_to_alloy_addr(uni_v2_usdc_weth) };
    let sushi_route = AmmSwapRoute::SushiswapV2 { pool: ethers_to_alloy_addr(sushi_v2_usdc_weth) };

    // Trade params
    let amount_in_eth: U256 = parse_units(env::var("TRADE_AMOUNT_ETH").unwrap_or_else(|_| "0.01".into()), "ether")?.into(); // 0.01 ETH
    // Gas price policy: prefer env GAS_PRICE_GWEI; otherwise use base_fee + 1 gwei from current block
    let latest_block = simulator.get_latest_block()?;
    let base_fee_wei = simulator.get_base_fee_at_block(latest_block).unwrap_or(10_000_000_000u128); // fallback 10 gwei
    let tip_gwei: u64 = env::var("PRIORITY_FEE_GWEI").ok().and_then(|v| v.parse().ok()).unwrap_or(1);
    ...
    println!("Block {} base_fee: {} wei | chosen gas_price: {} wei", latest_block, base_fee_wei, gas_price);
    let gas_approve = U256::from(120_000);
    let gas_swap = U256::from(300_000);

    // Create sim chain and get starting nonce from forked state
    ...
    let usdc_after_buy = chain.erc20_balance_of_on_fork(ethers_to_alloy_addr(usdc), ethers_to_alloy_addr(owner))?;
    ...
    // Sell USDC -> ETH on Sushi V2 (sell balance after buy)
    nonce += 1;
    ...
    let usdc_after_sell = chain.erc20_balance_of_on_fork(ethers_to_alloy_addr(usdc), ethers_to_alloy_addr(owner))?;
...
fn unsigned_to_typed(unsigned: tx_simulator::UnsignedTransaction, from: ethers::types::Address) -> ethers::types::transaction::eip2718::TypedTransaction {
    let mut tx: ethers::types::transaction::eip2718::TypedTransaction = (
        ethers::types::TransactionRequest::new()
            .from(from)
    ).into();
    if let Some(to) = unsigned.to { tx.set_to(ethers::types::NameOrAddress::Address(alloy_to_ethers_addr(to))); }
    if let Some(gas) = unsigned.gas { tx.set_gas(U256::from(gas)); }
    if let Some(value) = unsigned.value { tx.set_value(ethers::types::U256::from_dec_str(&value.to_string()).unwrap()); }
    if let Some(data) = unsigned.data { tx.set_data(EthersBytes::from(data.as_ref().to_vec())); }
    tx
}

fn sign_typed(wallet: &LocalWallet, typed: &ethers::types::transaction::eip2718::TypedTransaction) -> eyre::Result<TransactionSigned> {
...
fn alloy_to_ethers_addr(a: AlloyAddress) -> ethers::types::Address { ethers::types::Address::from_slice(a.as_slice()) }
fn ethers_to_alloy_addr(a: ethers::types::Address) -> AlloyAddress { AlloyAddress::from_slice(a.as_bytes()) }
fn ethers_to_alloy_u256(u: ethers::types::U256) -> AlloyU256 { AlloyU256::from_str_radix(&u.to_string(), 10).unwrap_or_default() }
