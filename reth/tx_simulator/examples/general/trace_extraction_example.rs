...
///
/// This example contrasts the call-trace helper with the full trace variant.
/// `simulate_unsigned_transaction_with_trace` mirrors reth's `/debug/trace_call` output,
/// while `simulate_unsigned_transaction_with_full_trace_at_block` enables per-opcode `struct_logs`
/// just like `/debug/trace_transaction` with step recording.
...
async fn main() -> Result<()> {
    ...
    if !result.call_trace.calls.is_empty() {
        ...
        println!(
            "  Available: CallFrame with {} child calls",
            result.call_trace.calls.len()
        );
    }
    ...
    let _deadline = U256::from(9999999999u64);
    let _recipient: Address = "0x0C96c602b1b332B8AB2093E5d72D804a24bd5689".parse()?;
    ...
    println!("• Opcode-by-opcode `struct_logs` (steps) for deep diagnostics");
