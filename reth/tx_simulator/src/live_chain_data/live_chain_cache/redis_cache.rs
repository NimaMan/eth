use crate::live_chain_data::live_data_registry::{self, ChainStateSnapshot};
...
impl LiveChainCache {
    ...
    pub async fn latest_block_number(&self) -> Result<Option<u64>> {
        ...
        let value: Option<String> = conn.get(live_data_registry::keys::latest_block_number_key()).await?;
        Ok(parse_u64(value))
    }
    ...
    pub async fn latest_chain_state_block_number(&self) -> Result<Option<u64>> {
        ...
        let value: Option<String> =
            conn.get(live_data_registry::keys::latest_chain_state_block_number_key()).await?;
        Ok(parse_u64(value))
    }

    pub async fn fetch_processed_block(
        &self,
        block_number: u64,
    ...
    pub async fn find_processed_transaction(
        ...
        if let Some(tx) = self
            .fetch_processed_tx(block_number, tx_hash)
            .await?
        {
            return Ok(Some(tx));
        }
    ...
    pub async fn store_chain_state_snapshot(
        &self,
        ...
        if self.retention > 0 && snapshot.block_number >= self.retention as u64 {
            ...
            pipe.cmd("DEL")
                .arg(live_data_registry::keys::chain_state_snapshot_key(evict_target));
        }
