...
impl TxSimulator {
    ...
    pub async fn simulate_signed_transaction(&self, tx: &TransactionSigned) -> Result<SimulationResult> {
        let latest_block = self.get_latest_block()?;
        self.simulate_signed_transaction_at_block(tx, latest_block).await
    }
