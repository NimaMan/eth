...
1. **Resolve block context**  
   - If a caller provides a `SealedHeader`, we reuse it. Otherwise the simulator queries
     `header_by_number` and waits for the canonical state snapshot (`history_by_block_number`)
     to become available.  
   - Context resolution is handled by `prepare_block_context`, which retries the state lookup
     briefly so it works in live feeds right after a block import.
...
3. **Execute via revm**  
   - The simulator wires up an inspector (plain parity-style, Geth call tracer, or full step
     recording) depending on the helper being called.  
...
* **`simulate_unsigned_transaction`** – parity-style inspector. Equivalent to `eth_call`; returns only `success`, `gas_used`, and an optional revert string.
...
* `prepare_block_context` – orchestrates header resolution (`fetch_block_header`) and retries
  `history_by_block_number` until the state snapshot is ready.
...
* `rust/tx_simulator/src/contract_method_simulator.rs` – uses `simulate_unsigned_transaction_with_trace`
  so view-call helpers can grab the top-level return bytes straight from the call tracer output.
* `rust/tx_processor/src/processed_tx_provider/provider.rs` – calls the full-trace variants to populate
