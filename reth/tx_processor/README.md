...
let route = AmmSwapRoute::UniswapV2 { pool: pool_address };
let buy = tx_builders::build_buy_swap(&route, buyer, token_out, U256::from(1e18 as u128), 50, u64::MAX);
```

## Examples

- `examples/pool_analysis/can_buy_sell_common_tokens_uniswap_v2.rs`
- `examples/pool_analysis/can_buy_sell_common_tokens_uniswap_v3.rs`
- `examples/pool_analysis/erc20_pool_tax_demo.rs`
- `examples/simulation/approval_mechanics_demo.rs`
- `examples/simulation/specific_token_investigation/*`

Run an example:

```bash
cargo run --example can_buy_sell_common_tokens_uniswap_v2 -p tx_processor
cargo run --example can_buy_sell_common_tokens_uniswap_v3 -p tx_processor
```

## Requirements

- Synced Reth node with database at `$RETH_DATADIR` (default: `~/.local/share/reth/mainnet`)
- Rust 1.70+

## Troubleshooting

EAGAIN / MDBX lock (code 11): Stop the running Reth node before simulation to avoid DB locks.

## Configuration

- `RETH_DATADIR` – Reth datadir path
- `ETH_RPC_URL` – Optional RPC for comparison/aux reads
- Others: example‑specific

## Notes

- This crate no longer keeps protocol router addresses in its `PoolType`; routing is centralized in `reth_chain_query::tx_builders`.
- Version bumped to 0.3.0 with centralized AMM builders, removed pool adapters, and unified tax calculator.
