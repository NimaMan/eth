...
///   - token_net: Net change for unknown tokens (checksum address -> U256 amount)
///   - currency_net: Net change for known currencies (symbol -> U256 amount in appropriate units)
...
impl AddressBalanceChange {
    ...
    /// Get token net change
    pub fn get_token_net(&self, token_address: &str) -> Option<U256> {
        self.token_net.get(token_address).copied()
    }
    
    /// Get currency net change
    pub fn get_currency_net(&self, symbol: &str) -> Option<U256> {
