...
impl PyChainQuery {
    ...
    fn get_total_supply(&self, token_address: &str, block_number: Option<u64>) -> PyResult<String> {
        let token = Address::from_str(token_address.trim_start_matches("0x"))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(
                format!("Invalid token address: {}", e)
            ))?;
        ...
        let supply = self.runtime.block_on(async move {
            chain_query.get_total_supply(token, block_number).await
        }).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(
    ...
    fn get_decimals(&self, token_address: &str, block_number: Option<u64>) -> PyResult<u8> {
        let token = Address::from_str(token_address.trim_start_matches("0x"))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(
                format!("Invalid token address: {}", e)
            ))?;
        ...
        let chain_query = self.inner.clone();
        self.runtime.block_on(async move {
            chain_query.token.get_decimals(token, block_number).await
        }).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(
