...
async fn run_simulation(simulator: Arc<TxSimulator>) -> Result<()> {
    ...
    println!(
        "Swapping {} WETH for USDC on SushiSwap",
    ...
    let input_buy_sushi_swap =
        encode_v2_swap_params(amount_in, amount_out_min, &path_buy, buyer_address);
    ...
    // 2. Add SushiSwap command and input
    let input_sell_sushi_swap =
        encode_v2_swap_params(tokens_received, U256::ZERO, &path_sell, buyer_address);
...
fn encode_execute(
    commands: alloy_primitives::Bytes,
    inputs: Vec<alloy_primitives::Bytes>,
