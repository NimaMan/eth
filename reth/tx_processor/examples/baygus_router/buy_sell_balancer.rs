...
async fn run_simulation(simulator: Arc<TxSimulator>) -> Result<()> {
    println!("🦄 Baygus Universal Router - Balancer V2 Buy/Sell Simulation");
    println!("===========================================================\n");
    ...
    // Command: BALANCER_SWAP (0x06)
    // Input: (bytes32 poolId, address assetIn, address assetOut, address recipient, uint256 amount, uint256 limit)
    let limit = U256::ZERO; // No slippage protection for sim
    let input_buy = encode_balancer_swap_params(
        pool_id,
        weth_address,
        bal_address,
        buyer_address,
    ...
    let input_sell = encode_balancer_swap_params(
        pool_id,
        bal_address,
        weth_address,
        buyer_address,
    ...
    // 1. Prepend CMD_TRANSFER_FROM for BAL
    let mut transfer_out_input = Vec::new();
    transfer_out_input.extend_from_slice(&pad_address(bal_address));
    ...
    // 2. Add Balancer Swap command and input
    commands_sell_vec.push(0x06); // CMD_BALANCER_SWAP
    inputs_sell_vec.push(Bytes::from(input_sell));
...
fn encode_balancer_swap_params(
    ...
    if let Some(changes) = processed_tx.address_balance_changes.get(&address) {
        ...
        for (addr_key, amount) in &changes.token_net {
            if let Ok(addr) = addr_key.parse::<Address>() {
                ...
                } else if addr == token2_address {
                    currency_changes.insert("mBAL".to_string(), *amount);
        ...
        for (currency, amount) in currency_changes {
            ...
            let display_amount = if currency == "ETH" || currency == "WETH" || currency == "mWETH" {
                ...
                format_amount(amount.unsigned_abs(), 18) // BAL has 18 decimals
            };
