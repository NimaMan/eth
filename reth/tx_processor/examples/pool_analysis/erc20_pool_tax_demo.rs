...
async fn main() -> Result<()> {
    ...
    let token_address = Address::from([
        0xa1, 0xaf, 0xff, 0xe3, 0xf4, 0xd6, 0x11, 0xd2,
        0x52, 0x01, 0x0e, 0x3e, 0xaf, 0x6f, 0x4d, 0x77,
        0x08, 0x8b, 0x0c, 0xd7
    ]); // RFI token: 0xa1afffe3f4d611d252010e3eaf6f4d77088b0cd7
    
    let pool_address = Address::from([
        0xb9, 0xca, 0x9f, 0x21, 0x36, 0x67, 0xff, 0xd2,
        0x21, 0xf0, 0x78, 0xec, 0xf3, 0xa7, 0x2d, 0xaf,
        0xe0, 0x4d, 0x45, 0xab
    ...
    let result = match check_can_buy_sell_pool(
        simulator,
        tx_processor,
        config,
    ).await {
    ...
    if result.is_tradeable {
        ...
        if result.buy_tax_percent < 0.0 {
            ...
            println!("  Buy Tax: {:.2}%{}", result.buy_tax_percent,
                if (result.buy_tax_percent - 1.0).abs() < 0.5 { " ✅ Close to expected 1%" } else { " ⚠️ Differs from expected 1%" });
        }
        ...
        if result.sell_tax_percent < 0.0 {
            ...
            println!("  Sell Tax: {:.2}%{}", result.sell_tax_percent,
                if (result.sell_tax_percent - 1.0).abs() < 0.5 { " ✅ Close to expected 1%" } else { " ⚠️ Differs from expected 1%" });
        }
        println!();
