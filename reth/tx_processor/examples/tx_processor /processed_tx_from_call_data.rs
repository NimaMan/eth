...
async fn main() -> Result<()> {
    ...
    match provider.process_transaction_from_call_data(call_request, Some(latest_block)).await {
        Ok(processed_tx) => {
            ...
            println!("  To: {}", processed_tx.to_address.map(|a| a.to_string()).unwrap_or_else(|| "Contract Creation".to_string()));
            println!("  Value: {} ETH", processed_tx.value.to_string());
            
            println!("\n⛽ Gas & Fees:");
            println!("  Gas used: {}", processed_tx.fees.gas_used);
            println!("  Gas price: {} gwei", processed_tx.fees.gas_price / U256::from(1_000_000_000u64));
            println!("  Transaction fee: {} wei", processed_tx.fees.txn_fee);
            
            println!("\n🔄 Transfers & Events:");
            println!("  ERC20 transfers: {}", processed_tx.erc20_transfers.len());
            println!("  ERC721 transfers: {}", processed_tx.erc721_transfers.len());
            println!("  ERC1155 transfers: {}", processed_tx.erc1155_transfers.len());
