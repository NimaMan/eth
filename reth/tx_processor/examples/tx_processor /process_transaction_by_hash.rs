...
async fn main() -> Result<()> {
    ...
    match provider.process_transaction_by_hash(tx_hash).await {
        Ok(tx) => {
            ...
            println!("  To: {:?}", tx.to_address.map(|a| a.to_string()).unwrap_or_else(|| "Contract Creation".to_string()));
            ...
            if !tx.approvals.is_empty() {
                ...
                for (i, approval) in tx.approvals.iter().enumerate() {
                    println!("  Approval #{}: {} approves {} to spend {}", 
                        i + 1, 
                        approval.owner, 
                        approval.spender, 
                        approval.amount
            ...
            if !tx.address_balance_changes.is_empty() {
                println!("\n💰 Address Balance Changes: {} addresses affected", tx.address_balance_changes.len());
            ...
            println!("  ✅ Address Balance Changes: {}", tx.address_balance_changes.len());
