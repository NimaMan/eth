...
// State validator with internal transaction tracking via RPC
// Based on the working json_state_validator.rs but adds internal transfer integration
...
use revm_tx_simulator_lib::{
    ...
    SimCacheDBForDiff,
    extract_internal_transfers_from_rpc, integrate_internal_transfers,
};
...
async fn main() -> Result<()> {
    ...
    // Setup providers (following the working example pattern)
    ...
    // Setup transaction environment (following working example)
    ...
    // Setup database and EVM (following working example)
    ...
    eprintln!("🚀 Executing transaction: {}", tx_hash_str);
    ...
    match mainnet_evm.transact_commit(tx_env.clone()) {
        Ok(execution_result) => {
            ...
            // Fetch internal transfers via RPC and integrate them
            match extract_internal_transfers_from_rpc(tx_hash_str, RPC_URL).await {
                Ok(internal_transfers) => {
                    eprintln!("📊 Found {} internal ETH transfers", internal_transfers.len());
                    
                    for (i, transfer) in internal_transfers.iter().enumerate() {
                        eprintln!("   Transfer {}: {:?} -> {:?}: {} ETH (depth: {}, success: {})", 
                                 i + 1, transfer.from, transfer.to,
                                 transfer.value.to_string().parse::<f64>().unwrap_or(0.0) / 1e18,
                                 transfer.depth, transfer.success);
                    }
                    
                    // Integrate internal transfers
                    state_changes = integrate_internal_transfers(state_changes, &internal_transfers);
                    eprintln!("📊 Updated state changes with internal transfers");
                }
                Err(e) => {
                    eprintln!("⚠️  Could not fetch internal transfers: {}", e);
                    eprintln!("📊 Using basic state changes only");
                }
            }
            
            eprintln!("📊 Found {} addresses with state changes", state_changes.len());
            
            // Convert to JSON format matching Python
            ...
            for (address, changes) in &state_changes {
                ...
                for (_token_addr, token_change) in &changes.token_net_changes {
                    ...
                    // Note: This assumes 18 decimals for all tokens, which isn't always true
