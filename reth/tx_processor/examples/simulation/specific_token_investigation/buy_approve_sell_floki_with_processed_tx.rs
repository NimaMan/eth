...
async fn main() -> Result<()> {
    ...
    println!("\n💰 Token Amounts:");
    println!(
        "  FLOKI received from buy: {}",
        format_floki_amount(floki_metrics.tokens_received_wei)
    );
    println!(
        "  ETH spent on buy: {}",
    ...
    if floki_metrics.sell_succeeded {
        println!(
            "  FLOKI sold: {}",
            format_floki_amount(floki_metrics.tokens_sold_wei)
        );
        println!(
            "  ETH received: {}",
        ...
        println!(
            "  Attempted to sell: {}",
            format_floki_amount(floki_metrics.tokens_attempted_to_sell)
        );
...
async fn execute_floki_trading_workflow(
    ...
    // Extra instrumentation: verify token_net keys and values for FLOKI
    {
        ...
        if let Some(changes) = buy_processed.address_balance_changes.get(&buyer) {
            ...
            let val_checksum = changes
                .token_net
                .get(&checksum_key)
                .copied()
                .unwrap_or(I256::ZERO);
            let val_debug = changes
                .token_net
                .get(&debug_key)
                .copied()
                .unwrap_or(I256::ZERO);
    ...
    println!("\n  🔍 Debug - Raw amounts:");
    println!("    FLOKI received (raw signed): {}", floki_signed);
    ...
    for (address, changes) in &buy_processed.address_balance_changes {
        ...
        if !changes.currency_net.is_empty() {
            ...
            for (currency, amount) in &changes.currency_net {
                let formatted_amount = if currency == "ETH" {
                    format_signed_eth(*amount)
                } else {
                    format!(
                        "{} {}",
                        format_signed_amount_with_decimals(*amount, 18),
                        currency
                    )
                };
                println!(
                    "        {}: {} (raw: {})",
                    currency, formatted_amount, amount
                );
            }
        }
        ...
        if !changes.token_net.is_empty() {
            ...
            for (token_key, amount) in &changes.token_net {
                let formatted_amount = if token_key.contains(&floki_address.to_string()[2..]) {
                    format_signed_floki(*amount)
                } else {
                    format!(
                        "{} tokens",
                        format_signed_amount_with_decimals(*amount, FLOKI_DECIMALS)
                    )
                };
                println!(
                    "        {}: {} (raw: {})",
                    token_key, formatted_amount, amount
                );
            }
        }
        ...
        if *address == buyer {
            ...
            if let Some(floki_change) = changes.token_net.get(&floki_address.to_string()) {
                ...
                println!("        Formatted: {}", format_signed_floki(*floki_change));
            }
    ...
    match buy_tax_result {
        TaxCalculationResult::Calculated { tax_basis_points } => {
            if let Some(tax_percentage) = buy_tax_result.as_percentage() {
                println!(
                    "    ✅ Buy tax calculated: {:.2}% ({} basis points)",
                    tax_percentage, tax_basis_points
                );
                if tax_basis_points > 0 {
                    println!("    ⚠️  FLOKI has a {:.2}% buy tax!", tax_percentage);
                    println!(
                        "       This means you receive {:.2}% fewer tokens than expected",
                        tax_percentage
                    );
                } else {
        ...
        TaxCalculationResult::InvalidSimulation { reason } => {
            ...
            let buyer_floki = buy_processed
                .get_address_token_balance_change(&buyer, &floki_address)
                .unwrap_or(I256::ZERO);
            let pool_floki = buy_processed
                .get_address_token_balance_change(&pool_address, &floki_address)
                .unwrap_or(I256::ZERO);
            ...
            println!(
                "      Buyer FLOKI change: {} ({})",
                buyer_floki,
                format_signed_floki(buyer_floki)
            );
            println!(
                "      Pool FLOKI change: {} ({})",
                pool_floki,
                format_signed_floki(pool_floki)
            );

            if pool_floki.is_positive() {
                println!("      ⚠️  Pool GAINED FLOKI - this indicates a tax/reflection mechanism");
                println!(
                    "      💡 FLOKI likely redistributes tokens to holders during transactions"
                );
            }
        }
    }
    ...
    let test_amounts = vec![("Full amount", metrics.tokens_received_wei)];

    println!("\n  🔍 Debug - Testing different sell amounts:");
    println!(
        "    Total received: {} ({} formatted)",
        metrics.tokens_received_wei,
        format_floki_amount(metrics.tokens_received_wei)
    );
    ...
    if let Some(output) = balance_result.call_trace.output {
        if output.len() >= 32 {
            ...
            println!(
                "    Formatted: {} FLOKI",
                format_floki_amount(actual_balance)
            );
    ...
    println!(
        "    Approve test caller: success={}, gas_used={}",
        approve_tester_res.success, approve_tester_res.gas_used
    );
    if !approve_tester_res.success {
        println!(
            "    Cannot proceed with targeted transferFrom test (approval failed): {:?}",
            approve_tester_res.revert_reason
        );
    }
    ...
    println!(
        "    Fund tx: success={}, gas_used={}",
        fund_res.success, fund_res.gas_used
    );
    ...
    println!(
        ...
        tf_small_res.success, tf_small_res.gas_used
    );
    ...
    println!(
        "    Result: success={}, gas_used={}",
        tf_full_res.success, tf_full_res.gas_used
    );
    ...
    println!(
        "    Attempting to transfer: {} raw ({} FLOKI)",
        test_transfer_amount,
        format_floki_amount(test_transfer_amount)
    );
    ...
    println!(
        "    Total received: {} raw ({} formatted)",
        metrics.tokens_received_wei,
        format_floki_amount(metrics.tokens_received_wei)
    );
    println!(
        "    Amount to sell: {} raw ({} formatted)",
        floki_amount_to_sell,
        format_floki_amount(floki_amount_to_sell)
    );

    metrics.tokens_attempted_to_sell = floki_amount_to_sell;

    println!(
        "    Will pass to create_sell_floki_transaction: {} raw ({} FLOKI)",
        floki_amount_to_sell,
        format_floki_amount(floki_amount_to_sell)
    );
    ...
    if let Some(data) = &sell_tx.data {
        ...
        if data.len() >= 4 {
            ...
            if data.len() >= 36 {
                ...
                println!(
                    "    Amount in (from calldata): {} raw ({} FLOKI)",
                    amount_in,
                    format_floki_amount(amount_in)
                );
            }
        }
    }
    ...
    if sell_result.success {
        ...
        println!(
            "      FLOKI: {} (sold)",
            format_signed_floki(floki_sold_signed)
        );
        println!(
            "      ETH: {} (received)",
        ...
        metrics.sell_failure_reason = sell_result
            .revert_reason
            .clone()
...
fn log_balance_changes_for_floki(processed_tx: &ProcessedTransaction, buyer: Address) {
    let mut has_changes = false;

    if let Some(eth_amount) = processed_tx.get_address_currency_balance_change(&buyer, "ETH") {
        let magnitude = eth_amount.unsigned_abs();
        if magnitude > U256::from(1_000_000_000_000u64) {
            has_changes = true;
            println!("    ETH: {}", format_signed_eth(eth_amount));
        }
    }
    ...
    if let Some(changes) = processed_tx.address_balance_changes.get(&buyer) {
        for (currency, amount) in &changes.currency_net {
            if currency != "ETH" && *amount != I256::ZERO {
                has_changes = true;
                println!(
                    "    {}: {}",
                    currency,
...
fn format_signed_eth(amount: I256) -> String {
    if amount == I256::ZERO {
        return "0 ETH".to_string();
    }
    let sign = if amount.is_negative() { "-" } else { "+" };
    format!("{}{}", sign, format_eth_amount(amount.unsigned_abs()))
}

fn format_signed_floki(amount: I256) -> String {
    if amount == I256::ZERO {
        return "0 FLOKI".to_string();
    }
    let sign = if amount.is_negative() { "-" } else { "+" };
    format!("{}{}", sign, format_floki_amount(amount.unsigned_abs()))
}

fn format_signed_amount_with_decimals(amount: I256, decimals: u8) -> String {
    if amount == I256::ZERO {
        return "0".to_string();
    }
    let sign = if amount.is_negative() { "-" } else { "+" };
