...
async fn main() -> Result<()> {
    ...
    info!("   Balance from helper: {} raw ({} FLOKI)", balance, format_floki_amount(balance));
    ...
    if balance != balance_check {
        error!("   ⚠️ BALANCE MISMATCH! Helper: {} vs Direct: {}", balance, balance_check);
    }
    ...
    if balance == U256::ZERO {
        ...
        info!("   Original balance at block start: {} raw", original_balance);
    } else {
    ...
    let approve_uniswap_tx = create_approve_transaction(
        buyer,
    ...
    if !approve_result.success {
        error!("Failed to approve Uniswap router: {:?}", approve_result.revert_reason);
        return Ok(());
    }
    ...
    if !approve_successful_result.success {
        error!("Failed to approve successful router: {:?}", approve_successful_result.revert_reason);
        return Ok(());
    }
    ...
    if treasury_handler_address != Address::ZERO {
        ...
        test_treasury_handler(&mut chain, buyer, treasury_handler_address, balance / U256::from(100)).await?;
    } else {
    ...
    if tax_handler_address != Address::ZERO {
        ...
        test_tax_handler(&mut chain, buyer, tax_handler_address, balance / U256::from(100)).await?;
    } else {
    ...
    info!("We have {} FLOKI, let's test different transfers", format_floki_amount(balance));
    
    // Test 1: Transfer to a random address
    let random_address = Address::from_str("0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb0")?;
    info!("\n1. Testing transfer to random address: {:?}", random_address);
    test_direct_transfer(&mut chain, buyer, random_address, U256::from(100_000_000_000u128)).await?;
    ...
    test_direct_transfer(&mut chain, buyer, pool_address, U256::from(100_000_000_000u128)).await?;
    
    // Test 3: Transfer to router address
    let router_address = Address::from_str(UNISWAP_V2_ROUTER)?;
    info!("\n3. Testing transfer to router address: {:?}", router_address);
    test_direct_transfer(&mut chain, buyer, router_address, U256::from(100_000_000_000u128)).await?;
    ...
    let test_amounts = vec![
        ...
        ("1000 FLOKI", U256::from(1000u64) * U256::from(10u64).pow(U256::from(FLOKI_DECIMALS))),
        ("10000 FLOKI", U256::from(10000u64) * U256::from(10u64).pow(U256::from(FLOKI_DECIMALS))),
...
async fn test_direct_pool_interaction(
    ...
    info!("   Pool reserves - Reserve0 (WETH): {}, Reserve1 (FLOKI): {}", reserve0, reserve1);
    ...
    info!("   Expected output: {} wei WETH for {} FLOKI", amount_out, format_floki_amount(floki_amount));
    ...
    if !transfer_result.success {
        ...
        error!("      Parameters: to={}, amount={}", FLOKI_WETH_PAIR, floki_amount);
        return Ok(false);
    }
    ...
    if swap_result.success {
        ...
        error!("   ❌ Direct pool swap failed: {:?}", swap_result.revert_reason);
        return Ok(false);
    }
}
    ...
    if result.success {
        ...
        if let Some(output) = result.call_trace.output {
            if output.len() >= 32 {
                ...
                info!("   Tax amount: {} ({}% of transfer)", tax, (tax * U256::from(100)) / amount);
            }
        }
    } else {
        error!("❌ getTax FAILED!");
...
async fn test_treasury_handler(
    ...
    info!("Calling beforeTransferHandler({:?}, {:?}, {})", from, to, amount);
    ...
    if result.success {
        info!("✅ beforeTransferHandler succeeded! Gas used: {}", result.gas_used);
