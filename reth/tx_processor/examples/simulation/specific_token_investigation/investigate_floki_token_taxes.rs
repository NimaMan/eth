...
async fn main() -> Result<()> {
    ...
    let floki_token = Address::from([
        0xcf, 0x0c, 0x12, 0x2c, 0x6b, 0x73, 0xff, 0x80,
        0x9c, 0x69, 0x3d, 0xb7, 0x61, 0xe7, 0xba, 0xeb,
        0xe6, 0x2b, 0x6a, 0x2e
    ]); // 0xcf0c122c6b73ff809c693db761e7baebe62b6a2e
    
    // FLOKI/WETH Uniswap V2 pool address
    let floki_pool = Address::from([
        0xca, 0x7c, 0x27, 0x71, 0xd2, 0x48, 0xdc, 0xbe,
        0x09, 0xea, 0xbe, 0x0c, 0xe5, 0x7a, 0x62, 0xe1,
        0x8d, 0xa1, 0x78, 0xc0
    ...
    println!("  Buy Result: success={}, gas_used={}", buy_result.success, buy_result.gas_used);
    ...
    let tokens_received = extract_tokens_received(&buy_processed, buyer_address, floki_token);
    ...
    if buy_tax.as_percentage().unwrap_or(0.0) > 0.5 {
        ...
        println!("  ⚠️  No significant tax detected. Tax handler might be inactive or rate is very low.");
    }
    ...
    let approve_tx = tx_builders::build_approve_for_route(
        &route,
        buyer_address,
        floki_token,
        U256::MAX,
    ...
    println!("  Approve Result: success={}, gas_used={}", approve_result.success, approve_result.gas_used);
    
    if !approve_result.success {
        println!("  ❌ Approve transaction failed: {:?}", approve_result.revert_reason);
        return Ok(());
    }
    ...
    println!("  Sell Result: success={}, gas_used={}", sell_result.success, sell_result.gas_used);
    
    if !sell_result.success {
        println!("  ❌ Sell transaction failed: {:?}", sell_result.revert_reason);
        ...
        let next_block_chain = simulator.start_simulation_chain(Some(block_number + 1)).await?;
        ...
        println!("  Delayed Sell Result: success={}, gas_used={}", delayed_sell_result.success, delayed_sell_result.gas_used);
        ...
        if delayed_sell_result.success {
            ...
            println!("  💰 Sell Tax: {:.2}%", sell_tax.as_percentage().unwrap_or(0.0));
            
        } else {
            println!("  ❌ Sell still fails with block delay: {:?}", delayed_sell_result.revert_reason);
