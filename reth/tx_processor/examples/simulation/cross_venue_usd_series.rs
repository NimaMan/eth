...
use reth_chain_query::common_addresses::{get_address_by_name, compute_uniswap_v2_pool, compute_sushiswap_pool, compute_uniswap_v3_pool};
...
async fn main() -> Result<()> {
    ...
    let end_block: u64 = if let Some(s) = args.get(1) { s.parse().unwrap_or(0) } else { 0 };
    ...
    let end_block = if end_block == 0 { processed.get_latest_block().await? } else { end_block };
    ...
    // Routes to consider
    let routes = vec![
        AmmSwapRoute::UniswapV2 { pool: compute_uniswap_v2_pool(weth, usdc) },
        AmmSwapRoute::SushiswapV2 { pool: compute_sushiswap_pool(weth, usdc) },
        AmmSwapRoute::UniswapV3 { pool: compute_uniswap_v3_pool(weth, usdc, 500), fee_tier: 500 },
        AmmSwapRoute::UniswapV3 { pool: compute_uniswap_v3_pool(weth, usdc, 3000), fee_tier: 3000 },
    ];
    let routes_usdt = vec![
        AmmSwapRoute::UniswapV2 { pool: compute_uniswap_v2_pool(weth, usdt) },
        AmmSwapRoute::SushiswapV2 { pool: compute_sushiswap_pool(weth, usdt) },
        AmmSwapRoute::UniswapV3 { pool: compute_uniswap_v3_pool(weth, usdt, 500), fee_tier: 500 },
        AmmSwapRoute::UniswapV3 { pool: compute_uniswap_v3_pool(weth, usdt, 3000), fee_tier: 3000 },
    ];

    // Output CSV
    fs::create_dir_all("artifacts").ok();
    let out_path = format!("artifacts/cross_venue_usd_series_{}_{}.csv", end_block, n_blocks);
    ...
    println!("Scanning blocks {}..={} (size_eth = {:.6})", start_block, end_block, size_eth);
    for b in start_block..=end_block {
        ...
        // USDC best
        if let Some(row) = scan_best_for_token(simulator.clone(), txp.clone(), usdc, &routes, b, eth_in_wei, size_eth, gas_price_gwei).await? {
            writeln!(f, "{b},USDC,{:.6},{:.6},{},{},{:.9},{:.2},{:.9},{:.2},{},{}",
                gas_price_gwei,
                size_eth,
                row.buy, row.sell, row.pnl_eth_raw, row.pnl_usd_raw, row.pnl_eth_exec, row.pnl_usd_exec, row.gas_total, row.eth_back
            )?;
        }
        // USDT best
        if let Some(row) = scan_best_for_token(simulator.clone(), txp.clone(), usdt, &routes_usdt, b, eth_in_wei, size_eth, gas_price_gwei).await? {
            writeln!(f, "{b},USDT,{:.6},{:.6},{},{},{:.9},{:.2},{:.9},{:.2},{},{}",
                gas_price_gwei,
                size_eth,
                row.buy, row.sell, row.pnl_eth_raw, row.pnl_usd_raw, row.pnl_eth_exec, row.pnl_usd_exec, row.gas_total, row.eth_back
            )?;
...
struct Row { buy: String, sell: String, pnl_eth_raw: f64, pnl_usd_raw: f64, pnl_eth_exec: f64, pnl_usd_exec: f64, gas_total: u64, eth_back: f64 }
...
async fn scan_best_for_token(
    ...
    for buy_route in routes.iter() {
        for sell_route in routes.iter() {
            ...
            if best.as_ref().map(|b| pnl_usd_exec > b.pnl_usd_exec).unwrap_or(true) {
                best = Some(Row {
