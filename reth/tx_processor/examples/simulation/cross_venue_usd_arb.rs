...
use reth_chain_query::common_addresses::{get_address_by_name, compute_uniswap_v3_pool, compute_sushiswap_pool};
...
async fn main() -> Result<()> {
    ...
    let block: u64 = args.get(1).and_then(|s| s.parse().ok()).unwrap_or(DEFAULT_BLOCK);
    let size_eth: f64 = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(DEFAULT_SIZE_ETH);
    let threshold_usd: f64 = args.get(3).and_then(|s| s.parse().ok()).unwrap_or(DEFAULT_THRESHOLD_USD);

    // Shared provider for base_fee
    let processed = Arc::new(ProcessedTxProvider::with_provider_factory(simulator.provider_factory().clone())?);
    ...
    let buy_usdc = AmmSwapRoute::UniswapV3 { pool: v3_500_usdc_pool, fee_tier: 500 };
    let sell_usdc = AmmSwapRoute::SushiswapV2 { pool: compute_sushiswap_pool(weth, usdc) };

    let v3_500_usdt_pool = compute_uniswap_v3_pool(weth, usdt, 500);
    let buy_usdt = AmmSwapRoute::UniswapV3 { pool: v3_500_usdt_pool, fee_tier: 500 };
    let sell_usdt = AmmSwapRoute::SushiswapV2 { pool: compute_sushiswap_pool(weth, usdt) };

    // Convert size_eth → wei (rounded)
    let eth_in_wei = U256::from(((size_eth.max(0.0)) * 1e18).round() as u128);

    println!("Cross-venue two-leg arb @ block {} (size = {:.6} ETH)", block, size_eth);
    println!("Base fee = {:.3} gwei, Tip = {:.3} gwei", base_fee_wei as f64 / 1e9, tip_gwei);

    // Single pair demo (V3 500 -> Sushi V2)
    let r_usdc = simulate_cross_venue_buy_approve_sell(
        simulator.clone(), txp.clone(), usdc, buy_usdc, sell_usdc, eth_in_wei, block
    ).await?;
    report("USDC (V3_500 -> SushiV2)", r_usdc, 6, base_fee_wei, tip_gwei, size_eth);

    let r_usdt = simulate_cross_venue_buy_approve_sell(
        simulator.clone(), txp.clone(), usdt, buy_usdt, sell_usdt, eth_in_wei, block
    ).await?;
    report("USDT (V3_500 -> SushiV2)", r_usdt, 6, base_fee_wei, tip_gwei, size_eth);

    // Scan all venue pairs for USDC and USDT at this block
    println!("\nScanning all venue pairs (V2, SushiV2, V3[500,3000]) at block {}...", block);
    scan_token(
        simulator.clone(), txp.clone(), usdc, "USDC", weth, base_fee_wei, tip_gwei, block, eth_in_wei, size_eth, threshold_usd
    ).await?;
    scan_token(
        simulator.clone(), txp.clone(), usdt, "USDT", weth, base_fee_wei, tip_gwei, block, eth_in_wei, size_eth, threshold_usd
    ).await?;

    Ok(())
}

fn report(symbol: &str, r: tx_processor::simulator::CrossVenueArbResult, token_decimals: u8, base_fee_wei: u128, tip_gwei: f64, size_eth: f64) {
    ...
    let usd_per_eth = if size_eth > 0.0 { tokens / size_eth } else { tokens };// ~1 USD per token
    ...
    println!("  gas: buy={} approve={} sell={} total={}", r.gas_buy, r.gas_approve, r.gas_sell, r.gas_total);
    ...
    let decision = if eth_pnl_raw >= 0.0 { "EXECUTE" } else { "SKIP" };
...
async fn scan_token(
    ...
    use reth_chain_query::common_addresses::{compute_uniswap_v2_pool, compute_sushiswap_pool, compute_uniswap_v3_pool};
    use reth_chain_query::tx_builders::amm_swap_route::AmmSwapRoute;

    let mut routes: Vec<AmmSwapRoute> = vec![
        AmmSwapRoute::UniswapV2 { pool: compute_uniswap_v2_pool(weth, token) },
        AmmSwapRoute::SushiswapV2 { pool: compute_sushiswap_pool(weth, token) },
        AmmSwapRoute::UniswapV3 { pool: compute_uniswap_v3_pool(weth, token, 500), fee_tier: 500 },
        AmmSwapRoute::UniswapV3 { pool: compute_uniswap_v3_pool(weth, token, 3000), fee_tier: 3000 },
    ];
    ...
    for (i, buy_route) in routes.iter().enumerate() {
        for (j, sell_route) in routes.iter().enumerate() {
            ...
            let res = simulate_cross_venue_buy_approve_sell(
                simulator.clone(), txp.clone(), token, buy_route.clone(), sell_route.clone(), amount_in_eth, block
            ).await?;
            let tokens = res.tokens_bought.to_string().parse::<f64>().unwrap_or(0.0) / 10f64.powi(6);
            ...
            let usd_per_eth = if size_eth > 0.0 { tokens / size_eth } else { tokens };
            let usd_pnl_raw = pnl_eth_raw * usd_per_eth;
            let pnl_eth_exec = pnl_eth_raw.max(0.0);
            let usd_pnl_exec = usd_pnl_raw.max(0.0);

            rows.push((format!("{:?}", buy_route), format!("{:?}", sell_route), pnl_eth_exec, usd_pnl_exec, eth_back, res.gas_total));

            if best.as_ref().map(|b| pnl_eth_exec > b.2).unwrap_or(true) {
                best = Some((format!("{:?}", buy_route), format!("{:?}", sell_route), pnl_eth_exec, usd_pnl_exec, res.gas_total));
            }
        }
    }
    ...
    println!("\n{} profitable pairs (threshold ≥ ${:.2}):", symbol, threshold_usd);
    let mut shown = 0usize;
    for (buy, sell, pnl_eth, usd_pnl, eth_back, gas) in rows.into_iter() {
        let pass = if threshold_usd > 0.0 { usd_pnl >= threshold_usd } else { pnl_eth >= 0.0 };
        if pass {
            println!(
