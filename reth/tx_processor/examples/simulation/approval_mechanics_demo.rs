...
async fn main() -> Result<()> {
    ...
    let scenario1_transactions = vec![
    ...
    match simulator.simulate_transaction_sequence(scenario1_transactions, options.clone()).await {
        Ok(result) => {
            ...
            if result.results.len() >= 1 && !result.results[0].success {
                println!("✅ EXPECTED BEHAVIOR: Swap failed because USDC is not approved for spending");
            } else {
    ...
    match simulator.simulate_transaction_sequence(scenario2_transactions, options.clone()).await {
        Ok(result) => {
            print_sequence_results("Scenario 2", &result);
            
            // Verify our expectation: all transactions should succeed
            if result.sequence_success {
                println!("✅ EXPECTED BEHAVIOR: Both transactions succeeded - approval enables swap");
            } else {
    ...
    let scenario3_transactions = vec![
        create_swap_eth_for_usdc_transaction(),
    ];
    
    match simulator.simulate_transaction_sequence(scenario3_transactions, options).await {
        Ok(result) => {
...
fn create_swap_usdc_for_eth_transaction() -> UnsignedTransaction {
    ...
    let data = format!("{}{}{}{}{}{}{}{}{}", 
        function_signature, amount_in, amount_out_min, path_offset, to_address, deadline,
...
fn create_swap_eth_for_usdc_transaction() -> UnsignedTransaction {
    ...
    let data = format!("{}{}{}{}{}{}{}{}", 
        function_signature, amount_out_min, path_offset, to_address, deadline,
...
fn print_sequence_results(scenario_name: &str, result: &SequentialSimulationResult) {
    ...
    for (i, tx_result) in result.results.iter().enumerate() {
        let status = if tx_result.success { "✅ SUCCESS" } else { "❌ FAILED" };
        let tx_name = match i {
            0 => if result.total_transactions == 1 { "SWAP" } else { "APPROVE" },
            1 => "SWAP", 
            2 => "ADDITIONAL",
            _ => "TRANSACTION",
        };
