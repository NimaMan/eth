...
async fn main() -> Result<()> {
    ...
    println!("\n💰 Token Amounts:");
    println!(
        "  PEPE received from buy: {}",
        format_pepe_amount(pepe_metrics.tokens_received_wei)
    );
    println!(
        "  ETH spent on buy: {}",
    ...
    println!(
        "  PEPE sold: {}",
        format_pepe_amount(pepe_metrics.tokens_sold_wei)
    );
    println!(
        "  ETH received: {}",
...
async fn execute_pepe_trading_workflow_with_processed_tx(
    ...
    println!(
        "    PEPE: +{} (received)",
        format_pepe_amount(metrics.tokens_received_wei)
    );
    ...
    println!(
        "  Selling amount: {}",
        format_pepe_amount(pepe_amount_to_sell)
    );
    ...
    println!(
        "    PEPE: -{} (sold)",
        format_pepe_amount(metrics.tokens_sold_wei)
    );
    println!(
        "    ETH: +{} (received)",
...
fn extract_buy_amounts_from_balance_changes(
    ...
    let eth_signed = processed_tx
        .get_address_currency_balance_change(&buyer, "ETH")
        .unwrap_or(I256::ZERO);
    let eth_wei = eth_signed.unsigned_abs();

    // Get PEPE received from token_net using token address
    let pepe_address = Address::from_str(PEPE_ADDRESS).unwrap();
    let pepe_signed = processed_tx
        .get_address_token_balance_change(&buyer, &pepe_address)
        .unwrap_or(I256::ZERO);
    let pepe_wei = pepe_signed.unsigned_abs();
...
fn log_balance_changes_for_pepe(processed_tx: &ProcessedTransaction, buyer: Address) {
    ...
    if let Some(eth_amount) = processed_tx.get_address_currency_balance_change(&buyer, "ETH") {
        ...
        if magnitude > U256::from(1_000_000_000_000u64) {
            has_changes = true;
            let sign = if eth_amount.is_negative() { "-" } else { "+" };
            println!("    ETH: {}{}", sign, format_eth_amount(magnitude));
        }
    }
    ...
    if let Some(pepe_amount) = processed_tx.get_address_token_balance_change(&buyer, &pepe_address) {
        if pepe_amount != I256::ZERO {
            ...
            println!(
                "    PEPE: {}{}",
                sign,
                format_pepe_amount(pepe_amount.unsigned_abs())
            );
        }
    }
        ...
        for (currency, amount) in &changes.currency_net {
            if currency != "ETH" && *amount != I256::ZERO {
                ...
                println!(
                    ...
                    currency,
                    sign,
                    format_amount_with_decimals(amount.unsigned_abs(), 18)
                );
            }
        }
    }
