...
use qarqa_core_types::{FundFlow, FlowType};
...
impl NetworkBuilder {
    ...
    pub fn add_node_metadata(
        ...
        self.node_metadata.insert(address, NodeMetadata {
            label,
            is_contract,
            entity_category,
        });
    ...
    pub fn build_centered_network(
        ...
        info!("Building network centered on {:?} with max depth {}", center, max_depth);
        ...
        info!("Built centered network with {} nodes and {} edges", 
              network.nodes.len(), network.edges.len());
