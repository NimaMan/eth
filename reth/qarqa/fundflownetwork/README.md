...
### Layer 1: High-Level Graph Exploration
- **Data source**: `tx_participants` and `addresses` tables (5-50ms queries)
- **Purpose**: Build preliminary UNDIRECTED graph structure and collect transaction hashes
- **Operations**: 
  - Find connected addresses (who transacted with whom)
  - Query entity types for routing decisions
  - Build undirected edges (just connections, no flow direction)
  - Select high-priority transactions for deep analysis
- **Output**:
  - Preliminary graph with nodes (addresses + entity types)
  - Undirected edges (address connections via tx_hash)
  - List of transaction hashes for Layer 2 processing
  - **NOTE**: No fund flow directions, no amounts, no balance calculations
...
OUTPUT: preliminary_graph (UNDIRECTED), priority_transactions

frontier = PriorityQueue(ordered_by=value)
visited = Set()
...
seed_txs = query("
    ...
    ORDER BY value DESC
    LIMIT ?
", seed_address, MIN_VALUE, BATCH_SIZE)
...
for tx in seed_txs:
    ...
    counterparty = (tx.from_address == seed_address) ? tx.to_address : tx.from_address
    frontier.push({
        tx_hash: tx.tx_hash,
...
// Explore using value-weighted BFS
while not frontier.empty() and graph.node_count() < MAX_NODES:
    ...
    // Add UNDIRECTED edge (just connection, no flow direction)
    graph.add_undirected_edge(current.from, current.to, {
        tx_hash: current.tx_hash,
        value: current.value  // For prioritization only
    })
    
    // Decide if we should analyze this transaction deeply
    should_analyze = should_process_tx_deeply(current, address_info)
    if should_analyze:
        priority_txs.append(current.tx_hash)
    
    // Decide if we should expand from this address
    should_expand = evaluate_address_expansion(address_info)
    
    if should_expand and current.depth < MAX_DEPTH - 1:
        // Get more transactions from counterparty
        new_txs = query_connected_transactions(current.counterparty, limit=50)
        for tx in new_txs:
            next_counterparty = (tx.from_address == current.counterparty) ? 
...
```
FUNCTION evaluate_address(address_info) -> (should_expand, should_process_deeply)
    
    // Dead ends - stop exploration
    if address_info.entity_type in ['CEX', 'BURN_ADDRESS']:
        return (False, False)
    
    // High-value targets - expand and analyze
    if address_info.entity_type in ['WHALE', 'PROTOCOL_TREASURY']:
        return (True, True)
    
    // Routers - limited expansion, deep analysis needed
    if address_info.entity_type in ['DEX_ROUTER', 'AGGREGATOR']:
        return (True, True)  // Need deep analysis to find actual recipients
    
    // Smart contracts - need investigation
    if address_info.is_contract and address_info.entity_type == 'UNKNOWN':
        return (True, True)
    
    // Regular EOA - expand if not too active
    if not address_info.is_contract:
        if address_info.tx_count_30d > 1000:
            return (False, True)  // Sample only
        else:
            return (True, True)
    
    return (True, False)  // Default: expand but no deep analysis
```
