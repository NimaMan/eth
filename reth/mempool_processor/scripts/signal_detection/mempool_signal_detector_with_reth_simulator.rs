...
async fn main() -> Result<()> {
    ...
    let tx_simulator = match tokio::time::timeout(
        ...
        RethSignedTxSimulator::new(&args.reth_datadir)
    ).await {
        Ok(Ok(simulator)) => {
        ...
        Err(_) => {
            error!("❌ Timeout creating Direct Reth simulator after 10 seconds!");
            return Err(eyre::eyre!("Direct Reth simulator creation timed out"));
        }
    };
