...
use mempool_processor::token_parameter_extraction::{
    calculate_buy_tax_from_movements, calculate_sell_tax_from_movements,
};
...
async fn main() -> Result<()> {
    ...
    for (pool_address_str, pool_state) in all_pools.iter() {
        ...
        if pool_state.eth_reserve < 0.01 {
            info!(
                "⏭️  [{}/{}] Skipping pool {} - insufficient liquidity ({:.6} ETH)",
        ...
        info!(
            "🔄 [{}/{}] Processing token {} in pool {}",
            processed, pool_count, pool_state.token_address, pool_address_str
        );
        ...
        let market_cap_eth = if let Some(ref info) = token_info {
            if let (Some(ref total_supply_str), Some(price)) =
                (info.total_supply.as_ref(), price_ratio)
            {
        ...
        let creator_address = token_info
            .as_ref()
        ...
        let total_supply = token_info
            ...
            .unwrap_or(0.0);
        let decimals = token_info.as_ref().and_then(|i| i.decimals).unwrap_or(18);
        let owner_address = token_info
            .as_ref()
            .map(|i| i.current_owner.as_str())
            .unwrap_or("");

        // Run buy/sell simulation
        let (can_buy, can_sell, buy_tax, sell_tax, error_msg, is_honeypot) = match simulator
            .simulate_sequence(token_address, pool_address, None)
            .await
        {
            Ok(result) => {
        ...
        let buy_tax_str = buy_tax
            .map(|t| format!("{:.2}", t))
            .unwrap_or_else(|| "".to_string());
        let sell_tax_str = sell_tax
            .map(|t| format!("{:.2}", t))
            .unwrap_or_else(|| "".to_string());
        let market_cap_str = market_cap_eth
            .map(|mc| format!("{:.6}", mc))
            .unwrap_or_else(|| "".to_string());
        let price_ratio_str = price_ratio
            .map(|pr| format!("{:.12}", pr))
            .unwrap_or_else(|| "".to_string());
    ...
    writeln!(
        summary_file,
        "Token Parameter Extraction and Tax Validation Summary"
    )?;
    writeln!(
        summary_file,
        "===================================================="
    ...
    writeln!(
        summary_file,
        "  Test buy amount: 0.01 ETH (with 0.001 ETH fallback)"
    )?;
    writeln!(
        summary_file,
        "  Buyer address: {}",
