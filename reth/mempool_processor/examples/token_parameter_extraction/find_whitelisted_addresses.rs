...
use alloy_rpc_types::{BlockId, TransactionRequest, Filter};
...
async fn find_successful_sellers(
    ...
    println!("📊 Finding successful sellers from blocks {} to {}", from_block, to_block);
...
async fn find_whitelist_slot(
    ...
    for (slot_num, addresses) in &slot_hits {
        ...
        let boolean_count = addresses.iter()
            .filter(|(_, v)| *v == U256::from(1))
            .count();
        
        println!("   Slot {}: {} addresses with data ({} boolean TRUE)", 
            slot_num, addresses.len(), boolean_count);
...
async fn verify_can_sell(
    ...
    match simulator.simulate_unsigned_transaction_at_block(approve_request, block).await {
        Ok(result) if result.success => {
            ...
            match simulator.simulate_unsigned_transaction_at_block(sell_request, block).await {
...
async fn main() -> Result<()> {
    ...
    if let Some(slot) = whitelist_slot {
        ...
        let all_whitelisted = scan_all_whitelisted(&provider, token, slot, &whitelisted, block).await?;
        
        // Final summary
        println!("\n{}", "=".repeat(60));
        println!("📊 SUMMARY");
        println!("\n🔓 Whitelisted Addresses Found: {}", all_whitelisted.len());
        ...
        println!("   2. Check any address: keccak256(address . {}) -> storage value", slot);
