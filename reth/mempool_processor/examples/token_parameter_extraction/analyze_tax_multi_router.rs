...
use mempool_processor::token_tracking::token_parameter_extraction::{
    PoolReserves, TaxCalculator, TokenInfo,
};
...
async fn fetch_token_info(
    ...
    let name_result = provider.call(name_tx).block(block_number.into()).await?;
    ...
    let symbol_result = provider.call(symbol_tx).block(block_number.into()).await?;
...
async fn test_router(
    ...
    match tax_calculator
        .calculate_taxes_via_simulation(
            buyer_address,
            token_info,
            pool_address,
            router.address,
        ...
        .await
    {
        Ok((buy_tax, sell_tax, tokens_bought, _tokens_sold)) => RouterTestResult {
            router_name: router.name.clone(),
            buy_tax: Some(buy_tax),
            sell_tax: if sell_tax >= 0.0 {
                Some(sell_tax)
            } else {
                None
            },
            ...
            tokens_bought,
            error_msg: if sell_tax < 0.0 {
                Some("Sell failed".to_string())
            } else {
                None
            },
...
async fn main() -> Result<()> {
    ...
    if args.len() < 4 {
        println!(
            "Usage: {} <token_address> <pool_address> <start_block> [end_block]",
            args[0]
        );
    ...
    let token_short = format!("{:?}", token_address)
        .chars()
        .take(8)
        .collect::<String>();
    let log_file_path = format!(
        "{}/multi_router_tax_{}_{}.log",
    ...
    println!(
        ...
        format_token_amount(token_info.total_supply, token_info.decimals)
    );
    println!();
    ...
    for block_number in start_block..=end_block {
        ...
        let working_routers: Vec<_> = block_results
            .iter()
        ...
        if !working_routers.is_empty() {
            ...
            let uniswap_works = block_results
                .get("Uniswap V2")
                .map(|r| r.buy_success || r.sell_success)
                .unwrap_or(false);
            let maestro_works = block_results
                .get("Maestro")
                .map(|r| r.buy_success || r.sell_success)
                .unwrap_or(false);
            let banana_works = block_results
                .get("Banana Gun")
                .map(|r| r.buy_success || r.sell_success)
                .unwrap_or(false);
