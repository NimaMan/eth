            ...
            .await
        {
            Ok((buy_tax, sell_tax, tokens_bought, tokens_sold)) => {
                ...
                if sell_tax >= 0.0 {
                    ...
                    writeln!(
                        log_file,
                        "{} | 0.01 ETH | {} | {:.1}% | {} | FAILED | - | {:.1} | Transfer failed",
                        block_number, tokens_bought, buy_tax, tokens_sold, elapsed_ms
                    )?;
                    println!(
                        "❌ Buy: {:.1}%, Sell: FAILED (Bought: {}, Tried: {}) - {:.1}ms",
                        buy_tax,
                        format_token_amount(tokens_bought, token_info.decimals),
                        format_token_amount(tokens_sold, token_info.decimals),
                        elapsed_ms
            ...
            Err(e) => {
                ...
                let short_error = if error_msg.contains("revert") {
                    if let Some(start) = error_msg.find("revert") {
                        ...
                        if let Some(end) = revert_part
                            .find('"')
                            .and_then(|i| revert_part[i + 1..].find('"').map(|j| i + j + 1))
                        {
                            &revert_part[..end + 1]
                        } else {
                            "Reverted"
                ...
                writeln!(
                    log_file,
                    "{} | - | - | - | - | FAILED | - | {:.1} | {}",
                    block_number, elapsed_ms, short_error
                )?;
            }
        }
