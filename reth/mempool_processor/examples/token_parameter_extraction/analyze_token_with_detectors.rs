...
async fn get_token_info(token_address: Address, block: Option<u64>) -> Result<(u8, String, String, U256)> {
...
async fn main() -> Result<()> {
    ...
    if args.len() < 3 {
        println!("Usage: {} <token_address> <pool_address> [block_number]", args[0]);
    ...
    let (decimals, symbol, name, total_supply) = get_token_info(token_address, block_number).await?;
    
    writeln!(log_file, "Configuration")?;
    writeln!(log_file, "-------------")?;
    writeln!(log_file, "Max Acceptable Buy Tax: {}%", tax_config.max_acceptable_buy_tax)?;
    writeln!(log_file, "Max Acceptable Sell Tax: {}%", tax_config.max_acceptable_sell_tax)?;
    writeln!(log_file, "Honeypot Sell Threshold: {}%", tax_config.honeypot_sell_threshold)?;
    writeln!(log_file)?;
    ...
    writeln!(log_file, "Total Supply: {} ({})", total_supply, format_token_amount(total_supply, decimals))?;
    ...
    writeln!(log_file, "Test Buy Amount: {} ETH", format_token_amount(config.test_buy_amount, 18))?;
    ...
    match simulator.simulate_sequence(token_address, pool_address, block_number).await {
        Ok(result) => {
            ...
            let tokens_received = if result.buy_result.success {
                ...
                result.buy_result.state_changes
                    ...
                    .map(|i256_val| if i256_val >= I256::ZERO { i256_val.unsigned_abs() } else { U256::ZERO })
                    .unwrap_or(U256::ZERO)
            } else {
                U256::ZERO
            };
            
            writeln!(log_file, "  Tokens Received: {} ({})", 
                tokens_received, 
            ...
            writeln!(log_file, "  ETH Received: {} ETH", format_token_amount(eth_received, 18))?;
            writeln!(log_file)?;
            ...
            if let (Some(buy), Some(sell)) = (buy_tax, sell_tax) {
                ...
                if buy > tax_config.max_acceptable_buy_tax as f64 || sell > tax_config.max_acceptable_sell_tax as f64 {
                ...
                writeln!(log_file, "  ❌ Cannot determine total tax due to sell failure")?;
            }
            
            // State Changes Summary
            writeln!(log_file, "\nState Changes Summary:")?;
            writeln!(log_file, "  Buy Transaction:")?;
            writeln!(log_file, "    Addresses Affected: {}", result.buy_result.state_changes.len())?;
            writeln!(log_file, "    Gas Used: {}", result.buy_result.gas_used)?;
            ...
            for (address, changes) in &result.buy_result.state_changes {
                ...
                writeln!(log_file, "        ETH Net: {:+.18} ETH (raw: {} wei)", eth_value, changes.eth_net)?;
            ...
            writeln!(log_file, "\n  Sell Transaction:")?;
            writeln!(log_file, "    Addresses Affected: {}", result.sell_result.state_changes.len())?;
            writeln!(log_file, "    Gas Used: {}", result.sell_result.gas_used)?;
