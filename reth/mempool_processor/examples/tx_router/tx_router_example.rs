...
async fn main() -> Result<()> {
    ...
    writeln!(log_file, "Transaction Router with Token Cache Log - {}", timestamp)?;
    writeln!(log_file, "=========================================")?;
    ...
    if !sample_pools.is_empty() {
        ...
        for (i, (pool_addr, pool_info)) in sample_pools.iter().take(5).enumerate() {
            ...
            writeln!(log_file, "       ETH Reserve: {:.4} ETH", pool_info.eth_reserve)?;
            writeln!(log_file, "       Token Reserve: {:.0}", pool_info.token_reserve)?;
            writeln!(log_file, "       Last Updated Block: {}", pool_info.last_updated_block)?;
        }
    }
            ...
            match &routing_result.category {
                ...
                TransactionCategory::Regular { is_transfer, is_approval } => {
                    ...
                    if processed < 10 || *is_transfer || *is_approval { // Log first 10 or interesting ones
                        ...
                        writeln!(log_file, "    Transfer: {}, Approval: {}", is_transfer, is_approval)?;
                    }
                }
            }
            ...
            if processed % 100 == 0 {
                ...
                if current_pools > initial_pools || current_creators > initial_creators {
                    info!("  Cache updated: {} pools (+{}), {} creators (+{})", 
                          ...
                          current_creators, current_creators - initial_creators);
                }
            }
    ...
    writeln!(log_file, "  Time Elapsed: {:.2}s", total_elapsed.as_secs_f64())?;
    writeln!(log_file, "  Processing Rate: {:.0} tx/sec", processed as f64 / total_elapsed.as_secs_f64())?;
    
    writeln!(log_file, "\nTransaction Categories:")?;
    writeln!(log_file, "  Contract Creations: {} ({:.1}%)", contract_creations, (contract_creations as f64 / processed as f64) * 100.0)?;
    writeln!(log_file, "  Creator Actions: {} ({:.1}%)", creator_actions, (creator_actions as f64 / processed as f64) * 100.0)?;
    writeln!(log_file, "  DEX Interactions: {} ({:.1}%)", dex_interactions, (dex_interactions as f64 / processed as f64) * 100.0)?;
    writeln!(log_file, "  Regular Transactions: {} ({:.1}%)", regular_txs, (regular_txs as f64 / processed as f64) * 100.0)?;
    
    writeln!(log_file, "\nPriority Analysis:")?;
    writeln!(log_file, "  High Priority: {} ({:.1}%)", high_priority, (high_priority as f64 / processed as f64) * 100.0)?;
    writeln!(log_file, "  Requiring Simulation: {} ({:.1}%)", requires_simulation, (requires_simulation as f64 / processed as f64) * 100.0)?;
    
    writeln!(log_file, "\nToken Cache Performance:")?;
    writeln!(log_file, "  Initial State: {} pools, {} creators, {} tokens", initial_pools, initial_creators, initial_tokens)?;
    writeln!(log_file, "  Final State: {} pools, {} creators, {} tokens", final_pools, final_creators, final_tokens)?;
    writeln!(log_file, "  New Pools Added: {}", final_pools.saturating_sub(initial_pools))?;
    writeln!(log_file, "  New Creators Added: {}", final_creators.saturating_sub(initial_creators))?;
    writeln!(log_file, "  New Tokens Added: {}", final_tokens.saturating_sub(initial_tokens))?;
    writeln!(log_file, "  Creator Cache Hits: {} / {} ({:.1}%)", 
             creator_cache_hits, creator_actions, 
             if creator_actions > 0 { (creator_cache_hits as f64 / creator_actions as f64) * 100.0 } else { 0.0 })?;
    writeln!(log_file, "  Pool Cache Hits: {} / {} ({:.1}%)", 
             pool_cache_hits, dex_interactions,
             if dex_interactions > 0 { (pool_cache_hits as f64 / dex_interactions as f64) * 100.0 } else { 0.0 })?;
    
    log_file.flush()?;

    // Console summary
    info!("\n📊 === Transaction Routing Complete ===");
    info!("✅ Processed: {} transactions in {:.2}s ({:.0} tx/sec)", 
          processed, total_elapsed.as_secs_f64(), processed as f64 / total_elapsed.as_secs_f64());
    info!("\n📈 Categories:");
    info!("   Contract Creations: {} ({:.1}%)", contract_creations, (contract_creations as f64 / processed as f64) * 100.0);
    info!("   Creator Actions: {} ({:.1}%)", creator_actions, (creator_actions as f64 / processed as f64) * 100.0);
    info!("   DEX Interactions: {} ({:.1}%)", dex_interactions, (dex_interactions as f64 / processed as f64) * 100.0);
    info!("   Regular: {} ({:.1}%)", regular_txs, (regular_txs as f64 / processed as f64) * 100.0);
    info!("\n💾 Token Cache:");
    info!("   Pools: {} → {} (+{})", initial_pools, final_pools, final_pools.saturating_sub(initial_pools));
    info!("   Creators: {} → {} (+{})", initial_creators, final_creators, final_creators.saturating_sub(initial_creators));
    info!("   Tokens: {} → {} (+{})", initial_tokens, final_tokens, final_tokens.saturating_sub(initial_tokens));
    info!("   Cache Hit Rate: {:.1}% creators, {:.1}% pools", 
          if creator_actions > 0 { (creator_cache_hits as f64 / creator_actions as f64) * 100.0 } else { 0.0 },
          if dex_interactions > 0 { (pool_cache_hits as f64 / dex_interactions as f64) * 100.0 } else { 0.0 });
