...
impl PerformanceStats {
    ...
    fn print_summary(&self) {
        ...
        info!("  Complete: {} ({:.1}%)", self.complete_data_count,
...
async fn run_performance_monitor() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Write CSV header
    writeln!(log_file, "timestamp,tx_hash,latency_us,is_complete,avg_latency_us,total_count")?;
