...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    writeln!(hash_file, "# Started: {}", start_timestamp.format("%Y-%m-%d %H:%M:%S.%3f %Z"))?;
    writeln!(hash_file, "# Target: {} transactions", args.tx_count)?;
    writeln!(hash_file, "################################################################################")?;
    writeln!(hash_file, "# Columns:")?;
    writeln!(hash_file, "#   1. Transaction Hash (0x...)")?;
    writeln!(hash_file, "#   2. Detection Latency (nanoseconds from socket read)")?;
    writeln!(hash_file, "#   3. Log Timestamp (when we logged this transaction)")?;
    writeln!(hash_file, "#   4. Elapsed Time (seconds since start)")?;
    writeln!(hash_file, "################################################################################")?;
    writeln!(hash_file, "")?;
    
    // Initialize client
    let client = MempoolFetcherIPCClient::new(Some("/home/nima/.local/share/reth/mainnet/reth.ipc"))?;
    client.start().await?;
    ...
    while total_processed < args.tx_count {
        ...
        if txs.is_empty() {
            ...
            let detection_ns_values: Vec<u64> = txs.iter()
                .map(|tx| tx.detection_ns)
                .collect();
            ...
            if total_processed / 1000 > (total_processed - txs.len() as u64) / 1000 {
                ...
                info!("Progress: {}/{} ({:.1}%) - Rate: {:.1} tx/sec", 
                      total_processed, args.tx_count, 
    ...
    let mut sorted_fetch_us: Vec<u64> = fetch_times.iter()
    ...
    writeln!(log_file, "total_time_seconds,{:.3}", total_time.as_secs_f64())?;
    writeln!(log_file, "throughput_tx_per_sec,{:.1}", total_processed as f64 / total_time.as_secs_f64())?;
    ...
    writeln!(log_file, "fetch_latency_avg_us,{}", avg_fetch_time.as_micros())?;
    ...
    info!("   Throughput: {:.1} tx/sec", total_processed as f64 / total_time.as_secs_f64());
    info!("\n📊 FETCH PERFORMANCE:");
    info!("   Total fetches: {}", fetch_num);
    info!("   Empty fetches: {} ({:.1}%)", empty_fetches, empty_fetches as f64 / fetch_num as f64 * 100.0);
    info!("   Full batches: {} ({:.1}%)", full_batches, full_batches as f64 / fetch_num as f64 * 100.0);
    ...
    writeln!(hash_file, "# Completed: {}", Local::now().format("%Y-%m-%d %H:%M:%S.%3f %Z"))?;
    writeln!(hash_file, "# Total Duration: {:.3} seconds", total_time.as_secs_f64())?;
    writeln!(hash_file, "# Total Transactions: {}", total_processed)?;
    writeln!(hash_file, "# Average Rate: {:.1} tx/sec", total_processed as f64 / total_time.as_secs_f64())?;
    writeln!(hash_file, "################################################################################")?;
