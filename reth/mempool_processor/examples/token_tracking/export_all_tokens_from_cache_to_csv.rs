...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Cache loaded: {} tokens, {} pools, {} creators", 
    ...
    for creator in all_creators.iter() {
        ...
        for token in tokens {
            ...
            let mut row = format!(
                ...
                token.sell_tax.map_or(String::new(), |t| format!("{:.2}", t)),
                token.tax_risk_score,
                token.ownership_renounced,
                token.renouncement_block.map_or(String::new(), |b| b.to_string()),
            ...
            for pool in pools.iter().take(3) {
                row.push_str(&format!(
                    ...
                    pool.trading_enabled_block.map_or(String::new(), |b| b.to_string())
                ));
            }
