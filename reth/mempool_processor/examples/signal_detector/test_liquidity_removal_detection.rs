...
async fn main() -> Result<()> {
    ...
    for (tx_hash, label) in test_cases {
        ...
        // Fetch raw tx
        let tx_data: serde_json::Value = match rpc.request(
        ...
        let block_opt = tx_data.get("blockNumber")
            .and_then(|v| v.as_str())
            .and_then(|s| u64::from_str_radix(s.trim_start_matches("0x"), 16).ok());
        if let Some(block) = block_opt {
            info!("🧪 Simulating liquidity removal at tx block {}...", block);
            match liquidity_sim.simulate_removal(unsigned.clone(), Some(block)).await {
                Ok(res) => {
                    println!("\n✅ Removal simulation (at block {})", block);
                    println!("  Success: {}", res.success);
                    println!("  Pool:   {:?}", res.pool_address.map(|a| format!("0x{}", hex::encode(a))));
