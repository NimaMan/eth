...
async fn main() -> eyre::Result<()> {
    ...
    info!("🚀 Mempool Performance Analyzer - 1K Transaction Test");
    ...
    // First, drain the initial mempool to get fresh transactions
    info!("🔄 Draining initial mempool transactions...");
    let initial_drain_start = Instant::now();
    let mut drain_count = 0;
    
    loop {
        let drain_txs = fetcher.get_transactions().await?;
        if drain_txs.is_empty() {
            break;
        }
        drain_count += drain_txs.len();
        info!("  Drained {} transactions (total: {})", drain_txs.len(), drain_count);
    }
    
    info!("✅ Initial drain complete: {} transactions in {:.1}s", 
          drain_count, initial_drain_start.elapsed().as_secs_f64());
    
    // Now start the performance measurement
    info!("\n🎯 Starting 1K transaction performance measurement...");
    info!("⏳ Waiting for fresh mempool transactions...");
    ...
    while metrics.len() < target_count {
        ...
        info!("📦 Fetched {} new transactions in {:.1}ms", 
        ...
        for tx in transactions {
            ...
            let metric = TransactionMetrics {
                ...
                simulation_start_ms,
                simulation_end_ms,
                time_to_fetch_us: (fetch_timestamp - mempool_arrival) * 1000,
                simulation_time_us: simulation_duration.as_micros() as u64,
                state_extraction_time_us: 0, // Included in simulation time
                total_processing_time_us: ((simulation_end_ms - mempool_arrival) * 1000),
            ...
            if metrics.len() % 100 == 0 {
                info!("📊 Progress: {}/{} transactions processed", metrics.len(), target_count);
            }
    ...
    let stats = PerformanceStats {
        ...
        avg_time_to_fetch_ms: metrics.iter()
            .map(|m| m.time_to_fetch_us as f64 / 1000.0)
    ...
    info!("  Avg time to fetch:       {:.2} ms", stats.avg_time_to_fetch_ms);
