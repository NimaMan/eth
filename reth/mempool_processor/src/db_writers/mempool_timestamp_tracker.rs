    ...
    loop {
        ...
        if last_stats_report.elapsed() > Duration::from_secs(300) {
            info!(
                ...
                pending_timestamps.len(), stats.total_processed, stats.successful_updates, 
                stats.not_found, stats.expired, stats.errors
            );
...
async fn flush_pending(
    ...
    debug!("Attempting to flush {} pending mempool timestamps", total_pending);
    
    // Convert to vectors for the query
    let tx_hashes: Vec<String> = pending.keys().cloned().collect();
    let timestamps: Vec<DateTime<Utc>> = tx_hashes.iter()
        ...
        .await
    {
        Ok(rows) => {
            ...
            if updated_count > 0 {
                info!("Updated {} mempool timestamps, {} still pending", updated_count, pending.len());
            }
        }
        Err(e) => {
