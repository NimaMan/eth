...
impl TradingSignalRecord {
    ...
    pub fn from_trading_enabled_signal(signal: &TradingEnabledSignal) -> Self {
        Self {
            ...
            buy_tax_at_signal: Some(rust_decimal::Decimal::from_f64_retain(signal.buy_tax).unwrap_or_default()),
            sell_tax_at_signal: Some(rust_decimal::Decimal::from_f64_retain(signal.sell_tax).unwrap_or_default()),
            total_supply: None,
            owner_address: None,
...
impl TaxSignalRecord {
    ...
    pub fn from_tax_signal(signal: &TaxSignal) -> Self {
        Self {
            ...
            confidence: Some(rust_decimal::Decimal::from_f64_retain(signal.confidence).unwrap_or_default()),
            buy_tax_at_signal: signal.buy_tax.map(|tax| rust_decimal::Decimal::from_f64_retain(tax).unwrap_or_default()),
            sell_tax_at_signal: signal.sell_tax.map(|tax| rust_decimal::Decimal::from_f64_retain(tax).unwrap_or_default()),
...
impl LiquidityRemovalSignalRecord {
    ...
    pub fn from_liquidity_removal_signal(signal: &LiquidityRemovalSignal) -> Self {
        Self {
            ...
            detection_timestamp: chrono::Utc::now(),
            detection_tx_hash: signal.tx_hash.clone(),
            liquidity_removed_eth: signal.estimated_eth_removed.map(|v| rust_decimal::Decimal::from_f64_retain(v).unwrap_or_default()),
            liquidity_removed_token: None,
            remaining_liquidity_eth: None,
            remaining_liquidity_token: None,
            removal_percentage: None,
...
impl LpApprovalSignalRecord {
    ...
    pub fn from_lp_approval_signal(signal: &LpApprovalSignal) -> Self {
        ...
        Self {
            ...
            approval_amount: Some(rust_decimal::Decimal::from_str_exact(&signal.amount.to_string()).unwrap_or_default()),
            is_unlimited_approval: unlimited,
            approval_type: "LP_TOKEN".to_string(),
            previous_allowance: signal.previous_allowance.map(|v| rust_decimal::Decimal::from_f64_retain(v).unwrap_or_default()),
