...
impl DbLogger {
    ...
    pub async fn new(
        ...
        info!("DbLogger::new() called with host={}, port={}, dbname={}", host, port, dbname);
        
        let connection_string = format!(
            "postgresql://{}:{}@{}:{}/{}?application_name=mempool_processor",
            user, password, host, port, dbname
        );
        info!("DbLogger::new() - connection string prepared");
        
        let logger = DbLogger {
            client: Arc::new(Mutex::new(None)),
            connection_string,
        };
        info!("DbLogger::new() - logger struct created");
        
        // Initialize connection
        info!("DbLogger::new() - calling connect()...");
        logger.connect().await?;
        info!("DbLogger::new() - connect() returned successfully");
        
        info!("Database logger initialized successfully");
        info!("DbLogger::new() - returning logger instance");
        Ok(logger)
    }
