...
struct ChannelStats {
    ...
    /// Last time we warned about queue size
...
pub struct FullTxIpcClient {
    ...
    /// Channel statistics
    channel_stats: Arc<RwLock<ChannelStats>>,
    
    /// Queue size counter (tracks current queue depth)
...
impl FullTxIpcClient {
    ...
    pub fn new(socket_path: Option<&str>) -> Result<Self> {
        ...
        // Increased buffer size from 10k to 50k to handle burst traffic
        let (tx_sender, tx_receiver) = mpsc::channel(50000);
        info!("📦 Created IPC client with 50k transaction buffer capacity");
        ...
        Ok(Self {
            ...
            channel_stats: Arc::new(RwLock::new(ChannelStats::default())),
    ...
    pub async fn start_monitoring(&self) -> Result<()> {
        ...
        *self.is_connected.write().await = true;
        ...
        tokio::spawn(async move {
            if let Err(e) = Self::monitor_loop(stream, tx_sender, stats, channel_stats, queue_size).await {
                        ...
                        if let Some(tx_data) = notification.get("result") {
                            if !tx_data.is_null() {
                                ...
                                if let Ok(transaction) = serde_json::from_value::<Transaction>(tx_data.clone()) {
                                    ...
                                    // Send transaction and update channel stats
                                    match tx_sender.try_send(full_tx) {
                                        Ok(_) => {
                                            // Increment queue size
                                            let mut q_size = queue_size.write().await;
                                            *q_size += 1;
                                            let mut ch_stats = channel_stats.write().await;
                                            ch_stats.queue_size = *q_size;
                                            ch_stats.max_queue_size = ch_stats.max_queue_size.max(*q_size);
                                        }
                                        Err(e) => {
                                            warn!("Channel full, dropping transaction: {}", e);
                                            let mut ch_stats = channel_stats.write().await;
                                            ch_stats.dropped_count += 1;
                                            ch_stats.queue_size = 50000; // Channel is full
                                            ch_stats.max_queue_size = 50000;
                                        }
                                    }
                                    
                                    // Log progress with queue info
                                    if count % 100 == 0 {
                                        let stats_snapshot = stats.read().await;
                                        let ch_stats = channel_stats.read().await;
                                        let current_q_size = *queue_size.read().await;
                                        info!("IPC Full: {} transactions, avg latency: {}μs, queue: {}/50000, dropped: {}", 
                                              count, stats_snapshot.avg_latency_us, current_q_size, ch_stats.dropped_count);
                                        
                                        // Warn if queue is getting full
                                        if current_q_size > 40000 {
                                            warn!("⚠️ Channel queue is {}% full ({}/50000)!", 
                                                  (current_q_size * 100) / 50000, current_q_size);
                                        }
                                    }
                                }
                            }
    ...
    pub async fn get_full_transactions(&self, max_count: usize) -> Result<Vec<FullIpcTransaction>> {
        ...
        // Wait for at least one transaction with timeout
        match tokio::time::timeout(Duration::from_millis(25), receiver.recv()).await {
            Ok(Some(tx)) => {
                transactions.push(tx);
                // Decrement queue size
                let mut q_size = self.queue_size.write().await;
                if *q_size > 0 {
                    *q_size -= 1;
                }
            },
            Ok(None) => return Err(eyre!("IPC channel closed")),
            Err(_) => return Ok(transactions), // Timeout, return empty vector
        }
        
        // Try to get more transactions without blocking
        while transactions.len() < max_count {
            match receiver.try_recv() {
                ...
                Err(_) => break, // Either empty or disconnected, just stop
            }
        }
    ...
    /// Get queue info (current size, max capacity)
    pub async fn get_queue_info(&self) -> Result<(usize, usize)> {
        let current_size = *self.queue_size.read().await;
        Ok((current_size, 50000)) // 50k is our buffer size
    }
}
