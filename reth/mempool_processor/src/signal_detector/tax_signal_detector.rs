    ...
    pub fn detect(&self, sim_result: &SimulationResult) -> Vec<TaxSignal> {
        ...
        if buy_tax_exceeds_threshold || sell_tax_exceeds_threshold || cant_sell {
            let details = match (
                ...
                (true, true, _) => format!(
                    "High taxes - Buy: {:.1}%, Sell: {:.1}%",
                    calculated_buy_tax.unwrap_or(0.0),
                    calculated_sell_tax.unwrap_or(0.0)
                ),
                (true, false, _) => {
                    format!("High buy tax: {:.1}%", calculated_buy_tax.unwrap_or(0.0))
                }
                (false, true, _) => {
                    format!("High sell tax: {:.1}%", calculated_sell_tax.unwrap_or(0.0))
                }
    ...
    fn log_tax_detection(
        ...
        if let Some(ref log_path) = self.log_file_path {
            if let Ok(mut file) = OpenOptions::new().create(true).append(true).open(log_path) {
                ...
                let (can_buy, can_sell, buy_tax_error, sell_tax_error) =
