...
impl Default for ArrivalRecorderConfig {
    fn default() -> Self {
        Self { flush_interval: Duration::from_secs(5), batch_size: 1000 }
    }
}
...
impl MempoolArrivalRecorder {
    ...
    pub fn new(db: Arc<RethIndexDB>, writer: Arc<MempoolArrivalWriter>, cfg: ArrivalRecorderConfig) -> Self {
                ...
                match writer_clone.write_arrivals_by_hashes_ms_return_resolved(&batch) {
                    Ok(resolved) => {
                        if !resolved.is_empty() {
                            ...
                            for h in resolved { map.remove(&h); }
                        }
                    }
                    Err(e) => {
    ...
    pub fn record_hash_hex_ms(&self, hash_hex: &str) {
        ...
        if let Ok(bytes) = hex::decode(h) {
            if bytes.len() == 32 {
                ...
                map.entry(hash).and_modify(|v| { if ts_ms < *v { *v = ts_ms; } }).or_insert(ts_ms);
            }
        }
    }
}
