...
pub fn convert_nonblocking_to_transaction_view(
    tx: &MempoolTransaction,
) -> Result<EthersTransaction> {
    ...
    // Set value
    let value_str = data.get("value").and_then(|v| v.as_str()).unwrap_or("0x0");
    ...
    if let Some(gas_price_str) = data.get("gasPrice").and_then(|v| v.as_str()) {
        ethers_tx.gas_price = Some(U256::from_str_radix(
    ...
    // Set input data
    let input_str = data.get("input").and_then(|v| v.as_str()).unwrap_or("0x");
