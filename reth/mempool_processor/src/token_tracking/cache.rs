...
pub use super::types::{Token, Pool, Address, CacheConfig, TokenWithPools, TokenUpdate};
...
impl TokenTrackingCache {
    ...
    pub async fn get_primary_pool(&self, token: &Address) -> Option<Arc<Pool>> {
        ...
        pools.into_iter()
            .max_by(|a, b| {
                a.eth_reserve.partial_cmp(&b.eth_reserve)
    ...
    pub async fn batch_update(&self, update: TokenUpdate) -> UpdateResult {
        ...
        for (token_addr, token_with_pools) in update.data {
            ...
            token.total_liquidity = pools.values()
                .map(|p| p.eth_reserve)
                .sum();
            
            token.primary_pool = pools.values()
                .max_by(|a, b| a.eth_reserve.partial_cmp(&b.eth_reserve).unwrap_or(std::cmp::Ordering::Equal))
            ...
            creator_to_tokens.entry(token.creator_address.clone())
            ...
            for tax_setter in &token.tax_setter_addresses {
                ...
                creator_to_tokens.entry(tax_setter.clone())
                    .or_insert_with(HashSet::new)
                    .insert(token_addr.clone());
            }
            ...
            creator_to_tokens.entry(token.current_owner.clone())
                .or_insert_with(HashSet::new)
                .insert(token_addr.clone());
    ...
    pub async fn get_token_info(&self, address: &str) -> Option<Token> {
        self.get_token(&address.to_string()).await
            .map(|arc| (*arc).clone())
    }
...
mod tests {
    ...
    async fn test_basic_operations() {
        ...
        token_data.insert("0xTOKEN".to_string(), TokenWithPools {
            token,
            pools: pools_map,
        });
        ...
        let creator_token = cache.get_token_for_creator(&"0xCREATOR".to_string()).await.unwrap();
