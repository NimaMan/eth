...
impl UniswapV2Pool {
    ...
    pub async fn from_tokens(
        ...
        let usdt: Address = "0xdAC17F958D2ee523a2206206994597C13D831ec7"
            .parse()
            .unwrap();
        let weth_usdc_pool: Address = "0xB4e16d0168e52d35CaCD2c6185b44281Ec28C9Dc"
            .parse()
            .unwrap();
        let weth_usdt_pool: Address = "0x0d4a11d5EEaaC28EC3F61d100daF4d40471f1852"
            .parse()
            .unwrap();

        let pool_address =
            if (token_a == weth && token_b == usdc) || (token_a == usdc && token_b == weth) {
    ...
    fn compute_pair_address(token_a: Address, token_b: Address) -> PoolResult<Address> {
        ...
        let init_code_hash =
            hex::decode("96e8ac4277198ff8b6f785478aa9a39f403cb768dd02cbee326c3e7da348845f")
                .map_err(|_| {
                    crate::common::errors::KartalError::Pool(PoolError::InvalidPoolState {
                        reason: "Invalid init code hash".to_string(),
                    })
                })?;

        // Compute CREATE2 address
        let salt =
            ethers::utils::keccak256(encode(&[Token::Address(token0), Token::Address(token1)]));
...
impl Pool for UniswapV2Pool {
    ...
    async fn supports_pair(&self, token_a: Address, token_b: Address) -> PoolResult<bool> {
        ...
        Ok((pool.token0 == t0 && pool.token1 == t1) || (pool.token0 == t1 && pool.token1 == t0))
    }
