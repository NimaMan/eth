...
use clap::Parser;
use eth_kartal::{
    alert_processor::{Alert, AlertReceiver, ReceiverConfig},
    tx_executor::{ExecutorConfig, TransactionExecutor},
...
fn random_h256() -> ethers::types::H256 {
    ethers::types::H256::from(rand::random::<[u8; 32]>())
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(ref rabbitmq_url) = args.rabbitmq_url {
        info!(
            "RabbitMQ: {} (enhanced gas estimation enabled)",
            rabbitmq_url
        );
    } else {
    ...
    tokio::select! {
        ...
        let result = if args.test_mode {
            ...
            eth_kartal::tx_executor::ExecutionResult {
                alert_id: alert.id,
                tx_hash: Some(ethers::types::random_h256()),
                success: true,
                error: None,
