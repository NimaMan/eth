        ...
        while !self.shutdown.load(Ordering::Relaxed) {
            match self.run_receiver().await {
                ...
                Err(e) => {
                    ...
                    error!("Alert receiver crashed (restart #{}, consecutive failures: {}): {}", 
                    ...
                    if consecutive_failures > 10 {
                        error!("Too many consecutive failures, waiting {} seconds before retry", 
                               delay.as_secs());
                    }
            ...
            match socket.recv_msg(0) {
                Ok(msg) => {
                    ...
                    match SignedMessage::from_zmq_message(data) {
                        Ok(signed_msg) => {
                            match self.validator.validate_alert(&signed_msg.payload, &signed_msg.signature) {
                                Ok(alert) => {
                                    alerts_received += 1;
                                    ...
                                    match self.tx.try_send(alert) {
                                        ...
                                        Err(mpsc::error::TrySendError::Full(alert)) => {
                                            ...
                                            if matches!(alert.params.priority, crate::common::Priority::Critical) {
