...
impl AlertValidator {
    ...
    pub fn validate_alert(&self, alert_json: &str, signature: &str) -> Result<Alert, KartalError> {
        ...
        match hmac::verify(&self.signing_key, alert_json.as_bytes(), &signature_bytes) {
            Ok(_) => {
                ...
                // Parse the alert
                let alert: Alert = serde_json::from_str(alert_json)
                    .map_err(|e| KartalError::SerializationError(format!("Failed to parse alert: {}", e)))?;
            ...
            Err(_) => {
                ...
                Err(KartalError::Validation("Invalid HMAC signature".to_string()))
            }
        }
    }
    ...
    fn validate_alert_content(&self, alert: &Alert) -> Result<(), KartalError> {
        ...
        if alert.id.is_empty() {
            return Err(KartalError::Validation("Alert ID cannot be empty".to_string()));
        }
        ...
        if now > alert_time && (now - alert_time) > 300 {
            return Err(KartalError::Validation("Alert timestamp too old".to_string()));
        }
        
        // Alert should not be from the future
        if alert_time > now + 60 { // Allow 1 minute clock skew
            return Err(KartalError::Validation("Alert timestamp in future".to_string()));
        }
...
impl SignedMessage {
    ...
    pub fn from_zmq_message(msg: &str) -> Result<Self, KartalError> {
        ...
        if parts.len() != 2 {
            return Err(KartalError::Validation("Invalid message format".to_string()));
        }
        ...
        if !sig_part.starts_with("SIGNATURE:") {
            return Err(KartalError::Validation("Missing signature prefix".to_string()));
        }
        
        if !payload_part.starts_with("PAYLOAD:") {
            return Err(KartalError::Validation("Missing payload prefix".to_string()));
        }
...
mod tests {
    ...
    use std::str::FromStr;
    use crate::alert_processor::{ExecutionParams, Action};
    ...
    fn test_signed_message_format() {
        ...
        assert_eq!(zmq_msg, "SIGNATURE:base64signature|PAYLOAD:{\"test\":\"payload\"}");
