...
use crate::common::errors::{ExecutionError, KartalError};
use ethers::prelude::*;
...
impl RetryExecutor {
    ...
    pub async fn execute_transaction_with_retry(
        ...
        self.execute_with_retry("transaction execution", || async {
            ...
            // Sign transaction
            let signed_tx = wallet.sign_transaction(&tx).await.map_err(|e| {
                KartalError::Execution(ExecutionError::TransactionFailed {
            ...
            let receipt = tokio::time::timeout(Duration::from_secs(120), pending_tx)
                ...
                .map_err(|e| {
                    KartalError::Network(crate::common::errors::NetworkError::ProviderError(e))
                })?;
