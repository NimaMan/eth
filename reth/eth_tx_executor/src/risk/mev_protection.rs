...
impl MEVProtector {
    ...
    pub fn calculate_frontrun_gas_price(
        ...
        self.detected_tx_gas_prices
            .insert(alert_hash, detected_gas_price);
        ...
        let (base_fee, priority_fee) = match &self.config.strategy {
            MEVStrategy::GasMultiplier(multiplier) => {
                ...
                let our_gas_price =
                    U256::from((detected_gas_price.as_u128() as f64 * multiplier) as u128);
            ...
            MEVStrategy::Adaptive {
                base_fee,
                priority_multiplier,
            } => {
                let priority =
                    U256::from((detected_gas_price.as_u128() as f64 * priority_multiplier) as u128);
                (*base_fee, priority)
            }
    ...
    fn extract_gas_price_from_alert(
        &self,
        alert: &Alert,
        ...
        warn!("No gas price in alert {}, using network estimate", alert.id);
    ...
    pub fn should_use_bundle(&self, alert: &Alert) -> bool {
        ...
        matches!(
            ...
            crate::alert_processor::Priority::Critical
        )
    }
