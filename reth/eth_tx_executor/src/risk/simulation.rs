...
impl SimulationEngine {
    ...
    pub fn simulate_partial_sell(
        ...
        match &self.mode {
            SimulationMode::Off => SimulationResult {
                would_succeed: false,
                estimated_gas_cost_eth: 0.0,
                estimated_execution_time_ms: 0,
                estimated_pnl_eth: 0.0,
    ...
    pub fn get_virtual_portfolio(&self) -> Option<VirtualPortfolioStats> {
        self.virtual_portfolio
            .as_ref()
            .map(|portfolio| VirtualPortfolioStats {
    ...
    fn simulate_full_emergency_sell(
        ...
        let save_percentage = if execution_time < 1000 {
            0.3
        } else if execution_time < 5000 {
            0.2
        } else {
            0.1
        };
        ...
        portfolio
            .token_positions
            .insert(token_address, current_position - amount_eth);
