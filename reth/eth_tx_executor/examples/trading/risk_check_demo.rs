...
use eth_kartal::risk::{RiskConfig, RiskDecision, RiskManager};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match risk_manager.check_gas_cost(trade_amount, normal_gas) {
        RiskDecision::Allow => {
            info!(
                "✅ Gas cost acceptable: {} ETH ({:.1}% of trade)",
                normal_gas,
                (normal_gas / trade_amount) * 100.0
            );
        }
        RiskDecision::Block { reason } => {
            info!("❌ Blocked: {}", reason);
    ...
    match risk_manager.check_gas_cost(trade_amount, high_gas) {
        ...
        RiskDecision::Block { reason } => {
            info!("❌ Blocked: {}", reason);
            info!(
                "   Gas would be {:.1}% of trade value!",
                (high_gas / trade_amount) * 100.0
            );
        }
    }
