...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let private_key = std::env::var("KARTAL_KILIT")
        .expect("KARTAL_KILIT environment variable not set");
    ...
    for slippage in slippages {
        ...
        println!("{:.1}% slippage → Min output: {:.2} USDC", 
    ...
    let pool2 = pool_factory.find_best_pool(
        WETH.parse()?,
        USDT.parse()?,
    ).await?;
    ...
    println!("Exchange rate: 1 ETH = {:.2} USDT", 
    ...
    println!("Base fee: {} gwei", ethers::utils::format_units(block.base_fee_per_gas.unwrap_or_default(), 9)?);
