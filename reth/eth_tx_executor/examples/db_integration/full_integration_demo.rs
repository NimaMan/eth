...
async fn show_position_status(
    ...
    println!(
        "   Token Amount: {} USDC",
        token_amount.parse::<u64>().unwrap_or(0) / 1_000_000
    );
    println!(
        "   ETH Invested: {} ETH",
        eth_invested.parse::<u128>().unwrap_or(0) as f64 / 1e18
    );
...
async fn show_final_results(
    ...
    println!(
        "   Received: {} ETH",
        eth_received.parse::<u128>().unwrap_or(0) as f64 / 1e18
    );
