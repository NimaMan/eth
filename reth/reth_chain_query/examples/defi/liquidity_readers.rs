...
use eyre::Result;
use reth_chain_query::{RethQueryProvider, AmmSwapRoute, Address, B256};
...
async fn main() -> Result<()> {
    ...
    println!("Reth DB: {} | latest {}", reth_db, provider.get_latest_block()?);
    ...
    let routes = vec![
        AmmSwapRoute::UniswapV2 { pool: uni_v2_usdc_weth },
        AmmSwapRoute::SushiswapV2 { pool: sushi_v2_usdc_weth },
        AmmSwapRoute::UniswapV3 { pool: uni_v3_weth_usdc_500, fee_tier: 500 },
        AmmSwapRoute::UniswapV3 { pool: uni_v3_weth_usdc_3000, fee_tier: 3000 },
        AmmSwapRoute::UniswapV3 { pool: uni_v3_usdc_usdt_100, fee_tier: 100 },
        // Balancer: query pair reserves for WETH/USDC within the pool
        AmmSwapRoute::BalancerV2 { pool_id: bal_weth_usdc_pool_id, token_in: address!("C02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"), token_out: address!("A0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48") },
        // Curve: TriCrypto USDT (i=0) vs WETH (j=2)
        AmmSwapRoute::CurveV1 { pool: curve_tricrypto_pool, i: 0, j: 2, use_underlying: false },
    ];
    ...
    for route in routes {
        ...
        if let (Some(t0), Some(t1)) = (info.token0, info.token1) {
            println!("Tokens:   0x{:x} ({:?}) / 0x{:x} ({:?})",
                t0, info.token0_symbol, t1, info.token1_symbol);
        }
        match info.protocol {
            "Uniswap-V2" | "SushiswapV2" | "Balancer-V2" | "Curve-V1" => {
                println!("Reserves: reserve0={} reserve1={}", info.reserve0.unwrap_or_default(), info.reserve1.unwrap_or_default());
            }
            "Uniswap-V3" | "Uniswap-V4" => {
                println!("Liquidity: {} | tick={:?}", info.v3_liquidity.unwrap_or_default(), info.tick);
            }
            _ => {}
        }
