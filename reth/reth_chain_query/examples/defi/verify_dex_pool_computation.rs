...
fn test_uniswap_v3_pools(weth: Address, usdc: Address, usdt: Address, dai: Address) {
    ...
    print_verification(
        "WETH/USDC 0.3%",
        known_weth_usdc_3000,
        computed_weth_usdc_3000,
    );
    print_verification(
        "WETH/USDC 1%",
        known_weth_usdc_10000,
        computed_weth_usdc_10000,
    );
    print_verification(
        "WETH/USDT 0.05%",
        known_weth_usdt_500,
...
async fn test_dynamic_pool_discovery(
    weth: Address,
    usdc: Address,
    usdt: Address,
    dai: Address,
    ...
    let stable_pool_id = [
        0x06, 0xdf, 0x3b, 0x2b, 0xbb, 0x68, 0xad, 0xc8, 0xb0, 0xe3, 0x02, 0x44, 0x36, 0x92, 0x03,
        0x7e, 0xd9, 0xf9, 0x1b, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x63,
    ];
    ...
    match verify_balancer_pool_tokens(&simulator, stable_pool_id, dai, usdc, None).await {
        ...
        Ok(false) => {
            println!("  ❌ Balancer stable pool does not contain expected stablecoin tokens")
        }
