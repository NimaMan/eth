/// Comprehensive token information example
/// 
/// This example demonstrates getting complete token information including
/// metadata, supply, holder count, and other comprehensive details.
/// 
/// Run with: cargo run --example token_comprehensive_info

use reth_chain_query::{RethQueryProvider, Result};
use alloy_primitives::{Address, U256, utils::{format_units, format_ether}};
use std::str::FromStr;

#[tokio::main]
async fn main() -> Result<()> {
    println!("=== Comprehensive Token Information ===\n");
    ...
    match provider.get_token_total_supply(uni_token, None).await {
        Ok(supply) => {
            ...
            println!("Est. Market Cap:  ${:.2} (at ${:.2}/token)", market_cap, estimated_price);
        }
        Err(e) => {
    ...
    let notable_holders = vec![
        ...
        ("Uniswap Treasury", "1a9C8182C09F50C8318d769245beA52c32BE35BC"),
    ...
    println!("Is Contract:      {}", if is_contract { "Yes ✅" } else { "No ❌" });
    ...
    match provider.get_token_total_supply(uni_token, Some(earlier_block)).await {
        Ok(earlier_supply) => {
    ...
    println!("\n{}", "=".repeat(60));
    println!("✅ Comprehensive token analysis complete!");
