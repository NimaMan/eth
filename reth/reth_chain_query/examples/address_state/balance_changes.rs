...
async fn main() -> Result<()> {
    ...
    println!("Tracking Binance changes from block {} to {}", from_block, to_block);
    println!("Time period: ~{:.1} hours\n", (to_block - from_block) as f64 * 12.0 / 3600.0);
    ...
    let changes = provider.calculate_eth_and_token_balance_diff_between_blocks(
        binance,
    ...
    if total_inflow > total_outflow {
        println!("  Net Result:    +{} ETH (Accumulation)", 
            format_ether(total_inflow - total_outflow));
    } else {
        println!("  Net Result:    -{} ETH (Distribution)", 
