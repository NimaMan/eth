...
use alloy_primitives::{Address, U256, utils::format_ether, utils::format_units};
use std::str::FromStr;
...
const TOKENS: &[(&str, &str, u8, u64)] = &[
    ("USDC", "A0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48", 6, 6_445_813),  // USDC launch block
    ("USDT", "dAC17F958D2ee523a2206206994597C13D831ec7", 6, 4_634_748),  // USDT launch block
    ("DAI", "6B175474E89094C44Da98b954EedeAC495271d0F", 18, 8_928_158), // DAI launch block
];
...
async fn main() -> Result<()> {
    ...
    for (name, block) in MILESTONES {
        ...
        match provider.get_eth_or_token_balance_at_block(eth_foundation, None, *block).await {
            Ok(balance) => {
                println!("{:<25} {:>12} {:>20}", 
                    name, 
                    format!("#{}", block),
    ...
    for (symbol, token_addr, decimals, launch_block) in TOKENS {
        ...
        for (milestone, block) in checkpoints {
            match provider.get_eth_or_token_balance_at_block(vitalik, Some(token), *block).await {
                Ok(balance) => {
    ...
    println!("{:<15} {:>12} {:>20} {:>15}", "Time Ago", "Block", "ETH Balance", "Change");
    println!("{}", "-".repeat(70));
    ...
    for i in (0..=months).rev() {
        ...
        match provider.get_eth_or_token_balance_at_block(binance, None, block).await {
            Ok(balance) => {
    ...
    println!("{:<15} {:>20} {:>15}", "Period", "ETH Balance", "USDC Balance");
    println!("{}", "-".repeat(50));
    
    let usdc = Address::from_str("A0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48")?;
    
    for (block, label) in checkpoints {
        let eth_balance = provider.get_eth_or_token_balance_at_block(address, None, block).await?;
        let usdc_balance = provider.get_eth_or_token_balance_at_block(address, Some(usdc), block).await?;
    ...
    let eth_balance = provider.get_eth_or_token_balance_at_block(test_address, None, block).await?;
    println!("ETH at block {}:  {} ETH", block, format_ether(eth_balance));
    
    // USDC balance (token = Some(usdc))
    let usdc_balance = provider.get_eth_or_token_balance_at_block(test_address, Some(usdc), block).await?;
    println!("USDC at block {}: {} USDC", block, format_units(usdc_balance, 6)?);
