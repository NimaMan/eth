...
async fn main() -> Result<()> {
    ...
    let eth_balances = provider.get_eth_balances_for_multiple_addresses(
        exchange_addresses.clone(),
        None
    ).await?;
    ...
    for ((name, _), balance) in EXCHANGES.iter().zip(eth_balances.iter()) {
        println!("  {:12} {} ETH", format!("{}:", name), format_ether(*balance));
    ...
    let start = Instant::now();
    for addr in &exchange_addresses[..3] { // Just first 3 for comparison
        let _ = provider.get_account(*addr, None).await?;
    }
    let seq_time = start.elapsed();
    println!("Sequential time (3 addresses): {:?}", seq_time);
    println!("Batch is {:.1}x faster per address", 
    ...
    let usdc_balances = provider.batch_get_balances_for_token_holder_pairs(
        token_holder_pairs,
        None
    ).await?;
    ...
    let portfolio = provider.get_multiple_token_balances_for_address(
        binance,
        token_addresses,
        None
    ).await?;
    ...
    for (token_addr, balance) in portfolio {
        if let Some((symbol, _)) = TOKENS.iter()
    ...
    let historical_balances = provider.get_eth_balances_for_multiple_addresses(
        exchange_addresses[..3].to_vec(),
        Some(block)
    ).await?;
    ...
    let nonces = provider.get_transaction_counts_for_multiple_addresses(
        exchange_addresses.clone(),
        None
    ).await?;
