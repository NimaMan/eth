...
async fn main() -> Result<()> {
    ...
    println!("✅ Database fetch completed in {:.3}s", db_time.as_secs_f64());
    ...
    for (idx, (db_receipt, rpc_receipt)) in db_receipts.iter().zip(rpc_receipts.iter()).enumerate() {
        ...
        // Compare gas used
        let rpc_gas = rpc_receipt["gasUsed"].as_str()
            .and_then(|s| u64::from_str_radix(s.trim_start_matches("0x"), 16).ok())
            .unwrap_or(0);
        ...
        let rpc_cumulative = rpc_receipt["cumulativeGasUsed"].as_str()
            .and_then(|s| u64::from_str_radix(s.trim_start_matches("0x"), 16).ok())
            .unwrap_or(0);
        ...
        if db_receipt.cumulative_gas_used != rpc_cumulative {
            ...
            tx_mismatches.push(format!(
                ...
                db_receipt.cumulative_gas_used, 
                rpc_cumulative
            ));
        }
        
        // Compare logs count
        let rpc_logs = rpc_receipt["logs"].as_array()
            .map(|l| l.len())
            .unwrap_or(0);
    ...
    println!("  • Database fetch: {:.3}s ({} receipts)", 
        db_time.as_secs_f64(), db_receipts.len());
    println!("  • RPC fetch: {:.3}s ({} receipts)", 
        rpc_time.as_secs_f64(), rpc_receipts.len());
    println!("  • Speedup: {:.1}x faster", 
        rpc_time.as_secs_f64() / db_time.as_secs_f64());
    
    println!("\n📈 Throughput:");
    println!("  • Database: {:.0} receipts/second", 
        db_receipts.len() as f64 / db_time.as_secs_f64());
    println!("  • RPC: {:.0} receipts/second", 
    ...
    println!("  • Average logs per receipt: {:.2}", 
