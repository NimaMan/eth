...
impl BlockTimeConverter {
    ...
    pub async fn timestamp_to_block(&self, target_time: DateTime<Utc>) -> Result<u64> {
        ...
        let block = self.binary_search_block(0, latest_block, target_time).await?;
    ...
    pub async fn batch_block_to_timestamp(&self, block_numbers: &[u64]) -> Result<Vec<BlockTimestamp>> {
        let mut results = Vec::with_capacity(block_numbers.len());
        
        for &block_number in block_numbers {
            let timestamp = self.block_to_timestamp(block_number).await?;
            results.push(BlockTimestamp { block_number, timestamp });
        }
...
mod tests {
    ...
    fn test_estimate_timestamp() {
        ...
        let expected_seconds = super::ETHEREUM_GENESIS_TIMESTAMP + (block * super::AVERAGE_BLOCK_TIME) as i64;
        assert_eq!(estimated.timestamp(), expected_seconds);
    }
    
    #[test]
    fn test_estimate_block_number() {
        let timestamp = DateTime::from_timestamp(super::ETHEREUM_GENESIS_TIMESTAMP + 120_000, 0).unwrap();
