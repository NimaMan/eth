...
impl RethQueryProvider {
    ...
    pub async fn batch_get_balances_for_token_holder_pairs(
        ...
        let futures = requests.into_iter().map(|(token, holder)| {
            self.get_token_balance(token, holder, block)
        });
    ...
    pub async fn batch_get_storage(
        ...
        let futures = slots.into_iter().map(|slot| {
            self.get_storage(contract, slot, block)
        });
    ...
    pub async fn batch_get_portfolios(
        ...
        let futures = addresses.iter().map(|address| {
            self.get_portfolio(*address, tokens.clone(), block)
        });
    ...
    pub async fn batch_check_contracts(
        ...
        let futures = addresses.iter().map(|address| {
            self.is_contract(*address, block)
        });
    ...
    pub async fn get_transaction_counts_for_multiple_addresses(
        ...
        let futures = addresses.iter().map(|address| {
            self.get_account(*address, block)
        });
    ...
    pub async fn get_eth_balances_for_multiple_addresses(
        ...
        let futures = addresses.into_iter().map(|address| {
            self.get_eth_balance(address, block)
        });
