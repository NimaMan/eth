...
use super::common::{build_two_address_payload, call_uint256_view, contains_signature};
...
use self::deployment_replay::prepare_state_for_metadata;
...
impl RethQueryProvider {
    ...
    /// Get complete token metadata in one call. Returns `Ok(None)` when the
    /// bytecode does not expose the ERC-20 interface.
    ...
    /// Verify that the deployed bytecode exposes the ERC-20 surface area.
