...
pub async fn get_address_id(db: &PostgresDB, address: &str) -> Result<Option<i64>> {
    let id =
        sqlx::query_scalar::<_, i64>("SELECT address_id FROM eth_db.addresses WHERE address = $1")
            .bind(address)
...
pub async fn get_address_transactions(
    ...
    let address_id = get_address_id(db, address)
        .await?
...
pub async fn get_tx_participants(db: &PostgresDB, tx_hash: &str) -> Result<Vec<String>> {
    let rows = query(
        r#"
    ...
    let participants = rows.into_iter().map(|row| row.get("address")).collect();

    Ok(participants)
}

/// Get all participant address_ids in a transaction
pub async fn get_tx_participant_ids(db: &PostgresDB, tx_hash: &str) -> Result<Vec<i64>> {
...
pub async fn get_transactions_between_addresses(
    ...
    // Get address_ids
    let addr1_id = get_address_id(db, address1)
        .await?
        .ok_or_else(|| eyre::eyre!("Address not found: {}", address1))?;
    let addr2_id = get_address_id(db, address2)
        .await?
...
pub async fn count_address_transactions(db: &PostgresDB, address: &str) -> Result<i64> {
    let address_id = get_address_id(db, address)
        .await?
        .ok_or_else(|| eyre::eyre!("Address not found: {}", address))?;
