...
impl RethQueryProvider {
    ...
    pub async fn get_stablecoin_market_share(&self, block: Option<u64>) -> Result<StablecoinMarketAnalysis> {
        ...
        for stablecoin_info in STABLECOINS {
            ...
            let total_supply = self.execute_contract_method(
        ...
        let top_3_concentration: f64 = market_data.iter()
            .take(3)
            .map(|d| d.market_share_percent)
            .sum();
        
        let top_5_concentration: f64 = market_data.iter()
            .take(5)
            .map(|d| d.market_share_percent)
            .sum();
        
        // Calculate Herfindahl-Hirschman Index (HHI)
        let herfindahl_index: f64 = market_data.iter()
    ...
    pub async fn get_top_stablecoins(&self, n: usize, block: Option<u64>) -> Result<Vec<StablecoinMarketData>> {
    ...
    pub async fn get_stablecoins_by_unit(&self, block: Option<u64>) -> Result<MarketByUnitAnalysis> {
        let block_number = block.unwrap_or(self.get_latest_block()?);
        ...
        for stablecoin_info in STABLECOINS {
            ...
            units_map.entry(stablecoin_info.unit.to_string())
