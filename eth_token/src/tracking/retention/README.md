...
| `terminal_or_idle_50k` | Unbounded token index. Terminal scams and liquidity-removal pools are retained for 50,000 blocks after terminal evidence. Observed pools and tokens are dropped after 50,000 blocks without meaningful activity. | Historical PnL runs that need a final aggregate snapshot before dropping token state. |
