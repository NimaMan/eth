...
1. Phase 1: Transaction Collection
   - Uses tx_processor_py.TxProcessor for high-speed transaction processing
...
try:
    ...
    raise ImportError(
        "tx_processor_py not available. Please install with: "
...
class RustProcessedTransactionProvider:
    ...
    def __init__(self, 
        ...
        Args:
            reth_datadir: Path to Reth data directory
            logger: Optional logger instance
            cache_size: Maximum number of blocks/transactions to cache
        """
        
        self.logger = logger
        self.reth_datadir = reth_datadir
        self.cache_size = cache_size
        
        # Initialize Rust tx_processor
        try:
            self.tx_processor = tx_processor_py.TxProcessor(reth_datadir)
            if self.logger:
                stats = self.tx_processor.get_stats()
                self.logger.info(f"Initialized Rust tx_processor: {stats}")
    ...
    def get_processed_transactions_from_tx_hashes(
        ...
        try:
            ...
            # Log any failures
            if batch_results['failed_count'] > 0:
                if self.logger:
                    self.logger.warning(f"Failed to process {batch_results['failed_count']} transactions")
                    for failed in batch_results['failed']:
                        self.logger.debug(f"  Failed: {failed['hash'][:10]}...: {failed['error']}")
