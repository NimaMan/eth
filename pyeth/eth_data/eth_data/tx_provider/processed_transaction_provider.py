...
class ProcessedTransactionProvider:
    ...
    def __init__(self, w3=None, logger=None):
        ...
        self.block_processor = BlockProcessor(w3=self.w3, logger=logger, calculate_address_balance_changes=True)
    ...
    async def get_processed_transactions_from_block_numbers(
        ...
        for block_num in blocks_to_load:
            try:
                processed_block_result = await self.block_processor.process_block(block_num)
                processed_txs_in_block = processed_block_result.transactions if processed_block_result else []
