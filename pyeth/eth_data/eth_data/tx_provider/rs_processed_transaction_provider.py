...
from eth_data.tx_provider.processed_transaction_provider import ProcessedTransactionProvider
...
class RustProcessedTransactionProvider:
    ...
    def __init__(self, 
        ...
        # Initialize Rust tx_processor using PyReth singleton pattern
        try:
            # Use TxProcessor directly as it's available
            self.tx_processor = pyreth.TxProcessor()
            self.pyreth_client = None
            if self.logger:
                self.logger.info("Initialized Rust tx_processor via TxProcessor")
