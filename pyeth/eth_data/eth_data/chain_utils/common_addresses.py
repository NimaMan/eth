...
# Try to load ETF addresses from database
try:
    import os
    import psycopg2
    from psycopg2.extras import RealDictCursor
    ...
    # Connect to database and fetch ETF addresses
    with psycopg2.connect(**db_params) as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            # Fetch all ETF addresses from the database
            cur.execute("""
                SELECT address, name 
                FROM eth_db.addresses 
                WHERE entity_category = 'ETF' 
                   OR name ILIKE '%BlackRock%'
                   OR name ILIKE '%Grayscale%'
                   OR name ILIKE '%Fidelity%'
                   OR name ILIKE '%Franklin%'
                   OR name ILIKE '%VanEck%'
                   OR name ILIKE '%WisdomTree%'
                   OR name ILIKE '%Invesco%'
                   OR name ILIKE '%Bitwise%'
                ORDER BY name
            """)
            ...
            for row in cur.fetchall():
                ...
                if name:
                    ...
                    ETF_ADDRESSES_BY_NAME[name] = address
                    ETF_NAMES_BY_ADDRESS[address] = name
    
    print(f"Loaded {len(ETF_ADDRESSES_BY_NAME)} ETF addresses from database")
    
except Exception as e:
    print(f"Warning: Could not load ETF addresses from database: {e}")
    
    # Fallback to hardcoded major ETF addresses if database fails
    ETF_ADDRESSES_BY_NAME = {
        # BlackRock ETFs
        'BlackRock ETHA': '0x3877D4faDabDB513A370A4381bBB1509E60290bb',
        'BlackRock ETHA 2': '0xC2b40e86178d7611d4bF906f52B0174778a8aAA9',
        'BlackRock ETHA 3': '0x7E2a9D3B486CaeEd30312f5A20b02bA7b77E8e8F',
        
        # Grayscale ETFs
        'Grayscale ETHE': '0x17b153aa3abe80655b558e52e478f0b3968bff16',
        'Grayscale ETHE 2': '0xf404806324dec457a0c5a8fb49302c8707b48386',
        'Grayscale ETHE 3': '0x099206266a54d4d7a41fb5eb5b687ebf7ac153b6',
...
# Create a set of ETF addresses for quick lookup
ETF_ADDRESSES_SET = set(ETF_NAMES_BY_ADDRESS.keys())
