...
async def simulate_historical_transaction():
    ...
    tx_processor = TransactionProcessor(w3, calculate_address_balance_changes=True)
    ...
    actual_tx_data = tx_data_fetcher.get_transaction_data(tx_hash)
    
    # Process the actual historical transaction
    actual_dtx = await tx_processor.process_transaction_async(
        actual_tx_data['transaction'], 
        actual_tx_data['receipt'], 
        actual_tx_data['trace'],
        state_diff=True
    )
    
    print("   ✅ Processed actual transaction")
    print(f"   Actual ETH transfers: {len(actual_dtx.internal_transactions)}")
    print(f"   Actual ERC20 transfers: {len(actual_dtx.erc20_transfers)}")
    ...
    try:
        ...
        print(f"   Actual gas used: {actual_tx_data['receipt']['gasUsed']}")
        
        sim_success = not simulation_trace.get('failed', False)
        actual_success = actual_tx_data['receipt']['status'] == 1
        ...
        actual_eth_transfers = [
            ...
            for tx in actual_dtx.internal_transactions 
            if tx.value > 0
        ]
    ...
    print(f"   • Gas prediction accuracy: {'✅' if abs(int(simulation_trace.get('gasUsed', '0x0'), 16) - actual_tx_data['receipt']['gasUsed']) < 1000 else '❌'}")
