...
class LiveTokenPositionManager:
    ...
    def _update_position_from_token_data(self, position: TokenPositionData, token: LiveERC20Token):
        ...
        if position.position_state in [TokenPositionState.BUY_CONFIRMED, TokenPositionState.SELL_SUBMITTED]:
        ...
        elif position.position_state in [TokenPositionState.INIT, TokenPositionState.BUY_SUBMITTED]:
            # Pre-position updates - just track current price
            position.current_Xprice = token.sync_info.current_price_ratio
        
        # SELL_CONFIRMED positions don't need price/value updates
