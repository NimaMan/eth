//! Input validation and security measures for TX_FUND_FLOW
...
impl Default for ValidationRules {
    fn default() -> Self {
        Self {
            ...
            label_pattern: regex::Regex::new(r"^[a-zA-Z0-9_\-\. ]+$").unwrap(),
        }
    }
}
...
impl InputValidator {
    ...
    pub fn validate_address(&self, address_str: &str) -> QarqaResult<Address> {
        ...
        if !cleaned.starts_with("0x") && !cleaned.starts_with("0X") {
            return Err(QarqaError::InvalidInput(format!(
                "Address must start with '0x', got: '{}'",
                cleaned
            )));
        }
        ...
        if cleaned.len() != 42 {
            return Err(QarqaError::InvalidInput(format!(
                "Address must be 42 characters (0x + 40 hex), got {} characters",
                cleaned.len()
            )));
        }
    ...
    pub fn validate_transaction_hash(&self, hash_str: &str) -> QarqaResult<B256> {
        ...
        if !cleaned.starts_with("0x") && !cleaned.starts_with("0X") {
            return Err(QarqaError::InvalidInput(format!(
                "Transaction hash must start with '0x', got: '{}'",
                cleaned
            )));
        }
        ...
        if cleaned.len() != 66 {
            return Err(QarqaError::InvalidInput(format!(
                "Transaction hash must be 66 characters (0x + 64 hex), got {} characters",
                cleaned.len()
            )));
        }
    ...
    pub fn validate_depth(&self, depth: u32) -> QarqaResult<u32> {
        ...
        if depth > self.rules.max_depth {
            return Err(QarqaError::InvalidInput(format!(
                "Analysis depth {} exceeds maximum allowed depth of {}",
    ...
    pub fn validate_block_range(&self, start: u64, end: u64) -> QarqaResult<(u64, u64)> {
        ...
        if range > self.rules.max_block_range {
            return Err(QarqaError::InvalidInput(format!(
                "Block range {} exceeds maximum allowed range of {}",
    ...
    pub fn validate_label(&self, label: &str) -> QarqaResult<String> {
        if label.chars().any(|c| c.is_whitespace() && c != ' ') {
            return Err(QarqaError::InvalidInput(
                "Label contains invalid characters. Only alphanumeric, underscore, hyphen, dot, and space allowed".to_string()
            ));
        }
    ...
    pub fn validate_u256(&self, value_str: &str) -> QarqaResult<U256> {
        ...
        if cleaned.is_empty() {
            return Err(QarqaError::InvalidInput(
                "U256 value cannot be empty".to_string(),
            ));
        }
    ...
    pub fn sanitize_sql_identifier(&self, identifier: &str) -> QarqaResult<String> {
        ...
        if reserved
            .iter()
            .any(|&word| cleaned.eq_ignore_ascii_case(word))
        {
            return Err(QarqaError::InvalidInput(format!(
...
mod tests {
    ...
    fn test_address_validation() {
        ...
        assert!(validator
            .validate_address("742d35Cc6634C0532925a3b844Bc9e7595f5b899")
            .is_err());
        assert!(validator
            .validate_address("0xGGGG35Cc6634C0532925a3b844Bc9e7595f5b899")
            .is_err());
        assert!(validator
            .validate_address("0x742d35Cc6634C0532925a3b844Bc9e7595f5b89")
            .is_err());
    }
    ...
    fn test_sql_sanitization() {
        ...
        assert_eq!(
            validator.sanitize_sql_identifier("user_id").unwrap(),
            "user_id"
        );
    ...
    fn test_suspicious_pattern_detection() {
        ...
        assert!(security::detect_suspicious_pattern(
