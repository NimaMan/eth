...
async fn main() -> Result<()> {
    ...
    println!(
        "Config: max_depth={}, min_value=0 ETH (include all transactions)",
        config.max_depth
    );

    // Run discovery
    let discovery_result = explorer
        .explore(
            pool_address,
            config,
...
fn analyze_discovery_results(
    ...
    println!(
        ...
        result.priority_transactions.len()
    );
    println!("  Stop reason: {:?}", result.stop_reason);
    println!("  Time taken: {}ms", result.discovery_stats.time_taken_ms);
    println!(
        "  Total txs examined: {}",
        result.discovery_stats.total_txs_examined
    );
    println!(
        "  Addresses explored: {}",
        result.discovery_stats.addresses_explored
    );
    println!(
        "  CEX addresses found: {}",
        result.discovery_stats.cex_addresses_found
    );
    println!(
        "  DEX addresses found: {}",
        result.discovery_stats.dex_addresses_found
    );
    ...
    if let Some(creator_node) = result.graph.nodes.get(&creator) {
        println!(
            "✅ Creator found: entity_type={:?}, depth={}",
    ...
    if let Some(dest_node) = result.graph.nodes.get(&mexc) {
        println!(
            "✅ Destination found: entity_type={:?}, depth={}, explored={}",
    ...
    let found_liquidity_tx = result.graph.edges.iter().any(|edge| {
        format!("{:?}", edge.tx_hash)
            .to_lowercase()
            .contains(&liquidity_tx[2..])
    });
    ...
    if found_liquidity_tx {
        ...
        // Check priority txs
        let in_priority = result.priority_transactions.iter().any(|tx| {
            format!("{:?}", tx.tx_hash)
                .to_lowercase()
                .contains(&liquidity_tx[2..])
        });
