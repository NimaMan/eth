...
use std::str::FromStr;
use tx_fund_flow_core_types::EthMovementType;
...
async fn main() -> eyre::Result<()> {
    ...
    // Initialize tx_processor through the current provider API.
    ...
    match provider.process_transaction_by_hash(tx_hash).await {
        Ok(processed_tx) => {
            ...
            println!(
                ...
                processed_tx.address_balance_changes.len()
            );
                ...
                Ok(fund_flows) => {
                    ...
                    if !fund_flows.eth_movements.is_empty() {
                        ...
                        for (i, movement) in fund_flows.eth_movements.iter().enumerate() {
                            let amount_eth =
                                movement.amount.to_string().parse::<f64>().unwrap_or(0.0) / 1e18;
                            println!(
                                "   {}. {} → {}: {:.6} ETH ({:?})",
                                i + 1,
                    ...
                    // Check gas fees
                    let gas_cost = processed_tx.fees.gas_price
                        * alloy_primitives::U256::from(processed_tx.fees.gas_used);
                    ...
                    let has_gas_movement = fund_flows
                        .eth_movements
                        .iter()
                        .any(|m| matches!(m.movement_type, EthMovementType::Gas));
