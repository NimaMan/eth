...
use tx_fund_flow_eth_db_fetcher::{create_pool, DbConfig, TransactionFetcher};
...
async fn main() -> eyre::Result<()> {
    ...
    let rows = sqlx::query(schema_query).fetch_all(&db_pool).await?;
    ...
    let address_txs = tx_fetcher
        .get_address_transactions(pool_address, Some(liquidity_block), Some(1000))
        .await?;

    println!(
        "✅ Found {} transactions for pool (block ≤ {})",
        address_txs.len(),
        liquidity_block
    );
    ...
    let address_txs2 = tx_fetcher
        .get_address_transactions(db_format_address, Some(liquidity_block), Some(1000))
        .await?;
    println!(
        "   With DB format address: Found {} transactions",
        address_txs2.len()
    );
    println!();

    let mut relevant_txs: Vec<(String, i64)> = address_txs
        .iter()
    ...
    println!();
    println!(
        "✅ Step 1 complete. Found {} relevant transactions.",
        relevant_txs.len()
    );
