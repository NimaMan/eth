...
//! This module calculates how address balances change through fund flows
...
/// State change analyzer for tracking balance changes
#[derive(Clone)]
...
impl StateChangeAnalyzer {
    ...
    pub fn calculate_state_changes(
        &self,
        fund_flows: &[FundFlow],
    ) -> HashMap<Address, AddressStateChange> {
        debug!(
            "Calculating state changes for {} fund flows",
            fund_flows.len()
        );
        ...
        for flow in fund_flows {
            // Update sender state
            let sender_state = state_changes
                .entry(flow.from)
                .or_insert_with(|| AddressStateChange::new(flow.from));
            sender_state.subtract_flow(flow, self.eth_usd_rate);

            // Update receiver state
            let receiver_state = state_changes
                .entry(flow.to)
                .or_insert_with(|| AddressStateChange::new(flow.to));
        ...
        info!(
            "Calculated state changes for {} addresses",
            state_changes.len()
        );
        state_changes
    }
    ...
    pub fn filter_significant_changes(
        ...
        debug!(
            "Filtering state changes with min USD change: {}",
            min_usd_change
        );
...
impl AddressStateChange {
    ...
    fn update_block_range(&mut self, first: u64, last: u64) {
        if first < self.first_block && first > 0 {
...
mod tests {
    ...
    fn test_state_change_analyzer() {
        let analyzer = StateChangeAnalyzer::new().with_eth_rate(2500.0);

        let fund_flows = vec![FundFlow {
    ...
    fn test_filter_significant_changes() {
        ...
        assert!(significant.contains_key(
            &Address::from_str("0x2222222222222222222222222222222222222222").unwrap()
        ));
    }
