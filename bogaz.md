...
- `alpha/live/trading::LivePrioritySellPlanner` can build either a direct
  Uniswap V2 ETH/WETH sell route or a Uniswap V2 trading vault emergency-sell route,
...
| Route/calldata | `UniswapV2SellRouteBuilder` wraps direct V2 sells and `UniswapV2TradingVaultSellRouteBuilder` wraps Mode A vault emergency sells into `PreparedSellRoute`. Rust builders exist in `tx_simulator::tx_builders`. | Production min-out evidence, slippage policy, vault deployment config, and route selection still need to be wired. |
