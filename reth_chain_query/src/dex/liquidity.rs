...
pub struct PoolLiquidityInfo {
    ...
    pub price_1e18: Option<U256>,
    /// token1 per 1 token0, scaled by 1e18
    ...
    pub async fn get_route_liquidity(
        ...
        match *route {
            ...
            AmmSwapRoute::BalancerV2 {
                ...
                let (pool_addr, _spec) = self
                    .balancer_v2_get_pool_info(pool_id, Some(block_number))
                    .await?;
                let (tokens, balances, _last_block) = self
                    .balancer_v2_get_pool_tokens_and_balances(pool_id, Some(block_number))
                    .await?;
                // Map provided tokens to their balances
                let mut bal_map: std::collections::HashMap<Address, U256> =
                    std::collections::HashMap::new();
                for (i, t) in tokens.iter().enumerate() {
                    if let Some(b) = balances.get(i) {
                        bal_map.insert(*t, *b);
                    }
                }
            ...
            AmmSwapRoute::CurveV1 {
                ...
                let a = self
                    .curve_v1_get_coin(pool, i, use_underlying, Some(block_number))
                    .await
                    .unwrap_or(Address::ZERO);
                let b = self
                    .curve_v1_get_coin(pool, j, use_underlying, Some(block_number))
                    .await
                    .unwrap_or(Address::ZERO);
                let ra = self
                    .curve_v1_get_balance(pool, i, Some(block_number))
                    .await
                    .unwrap_or(U256::ZERO);
                let rb = self
                    .curve_v1_get_balance(pool, j, Some(block_number))
                    .await
                    .unwrap_or(U256::ZERO);
                // Sort for consistent orientation
                let (t0, t1, r0, r1) = if a <= b {
                    (a, b, ra, rb)
                } else {
                    (b, a, rb, ra)
                };
...
fn compute_price_from_reserves_1e18(r0: U256, r1: U256, d0: u8, d1: u8) -> U256 {
    ...
    if d0 >= d1 {
        ...
        if r0.is_zero() || adj.is_zero() {
            U256::ZERO
        } else {
            r1.saturating_mul(scale_18) / (r0.saturating_mul(adj))
        }
    }
}
...
fn compute_price_from_sqrt_price_1e18(sqrt_price_x96: U256, d0: u8, d1: u8) -> U256 {
    ...
    if d0 >= d1 {
        ...
        if adj.is_zero() {
            ...
            ratio.saturating_mul(scale_18) / adj
        }
    }
}
...
fn pow10_u256(exp: u32) -> U256 {
    ...
    for _ in 0..exp {
        out = out.saturating_mul(U256::from(10));
    }
    out
}
