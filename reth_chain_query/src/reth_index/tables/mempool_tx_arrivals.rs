...
impl MempoolTxArrivalTable {
    ...
    #[inline]
    pub fn encode_key(tx_number: u64) -> [u8; 8] {
        tx_number.to_be_bytes()
    }

    #[inline]
    pub fn decode_key(key: &[u8]) -> Result<u64> {
        if key.len() != 8 {
            return Err(eyre::eyre!("invalid key length"));
        }
        let mut arr = [0u8; 8];
    ...
    #[inline]
    pub fn encode_value(first_seen_ms: u64) -> [u8; 8] {
        first_seen_ms.to_be_bytes()
    }

    #[inline]
    pub fn decode_value(val: &[u8]) -> Result<u64> {
        if val.len() != 8 {
            return Err(eyre::eyre!("invalid value length"));
        }
        let mut arr = [0u8; 8];
