...
| fixed | multiple V4 tokens | multiple V4 pools | live warmup and V4-enabled range review | V4 pool status was being probed through a local Baygus executor path instead of the chain route used by mined swaps | V4 hooks and router-dependent token logic can treat different routers differently; until Universal Router or actual-route simulation exists, V4 swaps update pool state but do not set authoritative `can_buy` / `can_sell` labels | v4-route-equivalent-simulator |
