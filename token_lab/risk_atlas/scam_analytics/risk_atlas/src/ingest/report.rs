...
use crate::db::schema::{
    ...
    ModelReadinessItem, NumericStat, PoolEligibilityRow, ReviewExample, RiskAtlasRun,
};
...
pub struct RiskAtlasReportImport {
    ...
    pub event_evidence: Vec<EventEvidenceRow>,
...
fn build_decision_questions(
    ...
    if let Some(stat) = numeric_stat(
        ...
        questions.push(decision_question(
            ...
            stat.median
                .map(|value| format!("{} chain_block_delta median", fmt_number(value))),
            Some(format!(
                "Median scam label arrives after {} chain_block_delta; P90 is {} and P95 is {}. This is the base survival window for scam pools.",
    ...
    if let Some(stat) = numeric_stat(
        ...
        "First Pre-Removal LP Approval To Removal Chain Block Delta",
    ) {
        questions.push(decision_question(
            ...
            stat.median
                .map(|value| format!("{} chain_block_delta median", fmt_number(value))),
            Some(format!(
                "First pre-removal LP approval appears a median {} chain_block_delta before removal; P25 is {}, P75 is {}, P90 is {}. The current report measures chain_block_delta lead time, not active_observation_delta lead time yet.",
                fmt_optional_number(stat.median),
    ...
    if let Some(stat) = numeric_stat(
        ...
        questions.push(decision_question(
            ...
            stat.median
                .map(|value| format!("{} chain_block_delta median", fmt_number(value))),
            Some(format!(
                "The latest pre-removal LP approval is still a median {} chain_block_delta before removal. P25 is {}, which helps separate same-block approvals from earlier warning.",
                fmt_optional_number(stat.median),
                fmt_optional_number(stat.p25)
            )),
            "answered",
    ...
    let active_target_rows: i64 = active_targets
        .iter()
        .filter(|row| row.active_observation_delta.is_none())
        .map(|row| row.rows)
        .sum();
    ...
    questions.push(decision_question(
        ...
        "Model target",
        "What is the current active_observation_delta target base rate?",
        Some(format!("{}", percent(positive_rows, active_target_rows))),
        Some(format!(
            "{} of {} active_observation_delta rows are positive across the near-future direct-removal horizons. This is the class balance for the first conditional model.",
            fmt_count(positive_rows),
            fmt_count(active_target_rows)
        )),
        "answered",
        Some("active_observation_delta rows"),
...
fn append_numeric_stats(
    ...
    for (index, row) in table.rows.iter().enumerate() {
        ...
        out.push(NumericStat {
            section: section.to_string(),
            metric: canonical_numeric_metric(&row[0]),
...
fn append_active_horizon_rows(
    ...
    for (index, row) in table.rows.iter().enumerate() {
        ...
        out.push(ActiveTargetSummary {
            ...
            active_observation_delta: Some(parse_i32(&row[0])?),
            rows,
            unique_pools: None,
            positives: None,
            negatives: None,
