...
async fn main() -> Result<()> {
    ...
    while transactions.len() < target_count && block_num > latest_block - 100 {
        let block_result: Value = rpc_client
            .request(
    ...
    for (i, (_hash, _signed_tx, tx_data, sim_block)) in transactions.iter().enumerate() {
        ...
        let rpc_result: Result<Value, _> = rpc_client
            .request(
                "debug_traceCall",
                rpc_params![
                    unsigned_tx,
    ...
    info!(
        "   RPC Results: {} successes, average: {:?}",
        rpc_successes, avg_rpc_time
    );
                ...
                if tx_data["type"].as_str() == Some("0x2") {
                    ...
                    if let Some(max_fee) = tx_data["maxFeePerGas"].as_str() {
                        request.max_fee_per_gas = Some(
                            u128::from_str_radix(max_fee.trim_start_matches("0x"), 16).unwrap_or(0),
                        );
                    }
                    if let Some(priority_fee) = tx_data["maxPriorityFeePerGas"].as_str() {
                        request.max_priority_fee_per_gas = Some(
                            u128::from_str_radix(priority_fee.trim_start_matches("0x"), 16)
                                .unwrap_or(0),
                        );
                    }
                } else {
                    ...
                    if let Some(gas_price) = tx_data["gasPrice"].as_str() {
                        request.gas_price = Some(
                            u128::from_str_radix(gas_price.trim_start_matches("0x"), 16)
                                .unwrap_or(0),
                        );
                    }
                }
    ...
    info!(
        "   Direct Results: {} successes, average: {:?}",
        direct_successes, avg_direct_time
    );
    ...
    info!("\n📝 Test Conditions:");
    info!(
        "   - Same {} transactions tested via both methods",
        target_count
    );
    ...
    info!(
        "   - Simulator initialized once ({}ms), then reused",
        init_time.as_millis()
    );
