...
use super::{Result, TokenStateStoreError};

pub const TOKEN_STATE_DATABASE_CONFIG_KEY: &str = "databases.token_state.url";

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TokenStateStoreConfig {
    pub database_url: String,
}

impl TokenStateStoreConfig {
    ...
    pub fn from_eth_config(config: &EthConfigFile) -> Result<Self> {
        let database_url = config.required(TOKEN_STATE_DATABASE_CONFIG_KEY)?;
...
impl EthConfigFile {
    ...
    pub fn load(path: PathBuf) -> Result<Self> {
        let contents =
            fs::read_to_string(&path).map_err(|source| TokenStateStoreError::ConfigRead {
                path: path.clone(),
                source,
            })?;
        Ok(Self {
    ...
    pub fn required(&self, key: &'static str) -> Result<String> {
        self.optional(key)
            .ok_or_else(|| TokenStateStoreError::MissingConfigValue {
                key,
...
fn parse_toml_config(path: &PathBuf, contents: &str) -> Result<HashMap<String, String>> {
    ...
    let root =
        contents
            .parse::<toml::Value>()
            .map_err(|source| TokenStateStoreError::ConfigParse {
                path: path.clone(),
                source,
            })?;
