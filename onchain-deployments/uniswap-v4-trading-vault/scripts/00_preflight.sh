...
git status --short -- \
  onchain-deployments/uniswap-v4-trading-vault \
...
test -f onchain-deployments/uniswap-v4-trading-vault/config/mainnet.candidate.toml
test -f onchain-deployments/uniswap-v4-trading-vault/config/gas-policy.candidate.toml
test -f onchain-deployments/uniswap-v4-trading-vault/config/kartal-policy.candidate.json
test -f onchain-deployments/uniswap-v4-trading-vault/simulations/route-fixtures/eth-usdc-500-no-hook.json

jq -e '.schema == "uniswap_v4_trading_vault_kartal_policy_candidate_v1"' \
  onchain-deployments/uniswap-v4-trading-vault/config/kartal-policy.candidate.json >/dev/null
jq -e '.schema == "uniswap_v4_route_fixture_v1"' \
  onchain-deployments/uniswap-v4-trading-vault/simulations/route-fixtures/eth-usdc-500-no-hook.json >/dev/null
...
if rg -n "PRIVATE_KEY|API_TOKEN|SECRET|PASSWORD" onchain-deployments/uniswap-v4-trading-vault/config; then
