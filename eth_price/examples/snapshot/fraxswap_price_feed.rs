/// Fraxswap Price Feed Example
///
/// Shows ETH price from Fraxswap pools (Time-Weighted AMM - TWAMM)
/// Uses the modular tx_simulator and shared provider pattern
use eth_price::{FraxswapReader, PriceSource};
use reth_chain_query::provider_factory_from_datadir;
use std::time::Instant;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🥖 Fraxswap Price Feed - Time-Weighted AMM");
    println!("{}", "=".repeat(50));

    // Create shared provider factory
    let provider_factory = provider_factory_from_datadir("/home/nima/.local/share/reth/mainnet")?;

    // Initialize Fraxswap reader with shared provider
    let reader = FraxswapReader::from_provider(provider_factory)?;

    println!("\n📊 ETH Price from Fraxswap:");
    println!("{}", "-".repeat(50));
    ...
    println!("\n✅ Fraxswap price feed working with tx_simulator!");
    Ok(())
}
