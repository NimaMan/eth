...
ETH_TX_EXECUTOR_ENV="${ETH_TX_EXECUTOR_ENV_PATH:-${ETH_TX_EXECUTOR_ENV:-/home/nima/code/crypto/blockchains/eth/config.env}}"
...
if [[ -z "${ETH_TX_EXECUTOR_API_TOKEN:-}" && -f "$ETH_TX_EXECUTOR_ENV" ]]; then
  ETH_TX_EXECUTOR_API_TOKEN="$(awk -F= '/^ETH_TX_EXECUTOR_API_TOKEN=/{print $2; exit}' "$ETH_TX_EXECUTOR_ENV")"
fi
if [[ -z "${ETH_TX_EXECUTOR_API_TOKEN:-}" ]]; then
  echo "ETH_TX_EXECUTOR_API_TOKEN is required" >&2
  exit 1
fi
TOKEN="$ETH_TX_EXECUTOR_API_TOKEN"
  ...
  jq -n \
    ...
    '{
      ...
      metadata: {
        ...
        executor_boundary: "eth_tx_executor",
        tx_prep_version: 1
      }
    }' > "$output"
}

make_request "v4_vault_buy" "$BUY_DATA" "$BUY_VALUE_WEI" "350000" "$RUN_DIR/eth-tx-executor-buy-request.json"
make_request "v4_vault_emergency_sell" "$SELL_DATA" "0" "450000" "$RUN_DIR/eth-tx-executor-sell-request.json"

submit_request() {
  local label="$1"
  local request="$2"
  local body_file="$RUN_DIR/eth-tx-executor-$label-response-body.json"
  local code_file="$RUN_DIR/eth-tx-executor-$label-http-code.txt"
  ...
  curl -sS \
    ...
    -o "$body_file" \
    -w "%{http_code}" \
    -X POST "$ETH_TX_EXECUTOR_URL/eth/tx/direct-raw" \
    --data-binary "@$request" > "$code_file"
}

submit_request "buy" "$RUN_DIR/eth-tx-executor-buy-request.json"
submit_request "sell" "$RUN_DIR/eth-tx-executor-sell-request.json"

STATUS_BODY="$RUN_DIR/eth-tx-executor-status.json"
curl -fsS \
  -H "Authorization: Bearer $TOKEN" \
  "$ETH_TX_EXECUTOR_URL/eth/tx/status" > "$STATUS_BODY"

jq -n \
  --arg schema "uniswap_v4_trading_vault_eth_tx_executor_dry_run_v1" \
  --arg eth_tx_executor_url "$ETH_TX_EXECUTOR_URL" \
  --arg expect "$EXPECT" \
  --arg vault_address "$VAULT_ADDRESS" \
  --arg simulation_block "$SIMULATION_BLOCK" \
  --slurpfile status "$STATUS_BODY" \
  --slurpfile buy_request "$RUN_DIR/eth-tx-executor-buy-request.json" \
  --rawfile buy_response_raw "$RUN_DIR/eth-tx-executor-buy-response-body.json" \
  --rawfile buy_http_code "$RUN_DIR/eth-tx-executor-buy-http-code.txt" \
  --slurpfile sell_request "$RUN_DIR/eth-tx-executor-sell-request.json" \
  --rawfile sell_response_raw "$RUN_DIR/eth-tx-executor-sell-response-body.json" \
  --rawfile sell_http_code "$RUN_DIR/eth-tx-executor-sell-http-code.txt" \
  '{
    schema: $schema,
    eth_tx_executor_url: $eth_tx_executor_url,
    expect: $expect,
    vault_address: $vault_address,
  ...
  }' > "$RUN_DIR/eth-tx-executor-dry-run.json"

case "$EXPECT" in
  accepted)
    jq -e '.buy.http_code == 200 and .sell.http_code == 200' "$RUN_DIR/eth-tx-executor-dry-run.json" >/dev/null
    ;;
  policy-rejected)
    jq -e '.buy.http_code >= 400 and .sell.http_code >= 400' "$RUN_DIR/eth-tx-executor-dry-run.json" >/dev/null
    ;;
  *)
...
echo "eth_tx_executor_dry_run=$RUN_DIR/eth-tx-executor-dry-run.json"
