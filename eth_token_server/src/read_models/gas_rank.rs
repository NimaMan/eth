...
pub struct GasRankEstimateResponse {
    pub redis: RedisPublishedBlockWindow,
...
#[derive(Debug, Clone, Serialize)]
pub struct RedisPublishedBlockWindow {
    pub stream_key: String,
    pub requested_blocks: u64,
    pub available_stream_events: usize,
    pub backfilled_blocks: usize,
...
#[derive(Debug, Clone)]
struct RedisPublishedBlockEvent {
    stream_id: String,
    block_number: u64,
...
pub async fn estimate(
    ...
    let redis_blocks = load_published_blocks(
        &state.config.redis_url,
        &state.config.live_block_stream,
        lookback_blocks as usize,
    )
    .await?;
    if redis_blocks.is_empty() {
        return Err(eyre!(
            "no published blocks found in redis stream {}",
            state.config.live_block_stream
        ));
    }
    let redis_event_count = redis_blocks.len();
    let published_blocks = expand_published_block_window(redis_blocks, lookback_blocks as usize);
    let backfilled_blocks = published_blocks.len().saturating_sub(redis_event_count);
    ...
    if sampled.is_empty() {
        return Err(eyre!("no redis-published blocks could be loaded from Reth"));
    }
    ...
    Ok(GasRankEstimateResponse {
        redis: RedisPublishedBlockWindow {
            stream_key: state.config.live_block_stream.clone(),
            requested_blocks: lookback_blocks,
            available_stream_events: redis_event_count,
            backfilled_blocks,
            ...
            sampled_transactions: samples
                ...
                .sum::<usize>(),
            latest_stream_id: published_blocks
                .first()
                .map(|event| event.stream_id.clone()),
            latest_block: published_blocks.first().map(|event| event.block_number),
            latest_block_hash: published_blocks
                .first()
                .and_then(|event| event.block_hash.clone()),
            latest_processed_at_ms: published_blocks
                .first()
                .and_then(|event| event.processed_at_ms),
            block_numbers: published_blocks
                .iter()
...
async fn load_published_blocks(
    redis_url: &str,
    stream_key: &str,
    count: usize,
) -> Result<Vec<RedisPublishedBlockEvent>> {
    let client = Client::open(redis_url)
        .map_err(|err| eyre!("failed to create redis client for {redis_url}: {err}"))?;
    let mut conn = ConnectionManager::new(client)
        .await
        .map_err(|err| eyre!("failed to connect to redis {redis_url}: {err}"))?;
    let reply: StreamRangeReply = redis::cmd("XREVRANGE")
        ...
        .await
        .map_err(|err| eyre!("failed to read redis stream {stream_key}: {err}"))?;

    let mut events = reply
        .ids
        .into_iter()
        .filter_map(stream_id_to_event)
        .collect::<Vec<_>>();
    events.dedup_by_key(|event| event.block_number);
    Ok(events)
}

fn expand_published_block_window(
    events: Vec<RedisPublishedBlockEvent>,
    count: usize,
) -> Vec<RedisPublishedBlockEvent> {
    ...
    (0..count)
        .filter_map(|offset| {
            ...
            Some(
                by_block
                    .remove(&block_number)
                    .unwrap_or(RedisPublishedBlockEvent {
...
fn stream_id_to_event(id: StreamId) -> Option<RedisPublishedBlockEvent> {
    let block_number = value_to_u64(id.map.get("block_number")?).ok()?;
    Some(RedisPublishedBlockEvent {
        stream_id: id.id,
        block_number,
        block_hash: id
            .map
...
fn history_row(
    event: &RedisPublishedBlockEvent,
...
fn value_to_u64(value: &Value) -> Result<u64> {
    u64::from_redis_value(value).map_err(|err| eyre!("invalid redis integer field: {err}"))
}

fn value_to_string(value: &Value) -> Result<String> {
    String::from_redis_value(value).map_err(|err| eyre!("invalid redis string field: {err}"))
}
