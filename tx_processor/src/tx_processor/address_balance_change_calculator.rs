use super::data_models::balance_changes::{AddressBalanceChange, TokenMovement, TokenMovements};
...
fn subtract_as_i256(in_amount: U256, out_amount: U256) -> I256 {
    if in_amount >= out_amount {
        let diff = in_amount - out_amount;
        u256_to_i256(diff)
    } else {
        let diff = out_amount - in_amount;
        let signed = u256_to_i256(diff);
        signed.checked_neg().unwrap_or(I256::MIN)
    }
}
        ...
        for addr in all_addrs {
            ...
            if let Some(currency_movs) = self.currency_movements.get(addr) {
                for (currency_symbol, movements) in currency_movs {
                    let currency_in: U256 = movements
                        .incoming
                        .values()
                        .map(|e| e.amount)
                        .fold(U256::ZERO, |acc, x| acc + x);
                    let currency_out: U256 = movements
                        .outgoing
                        .values()
                        .map(|e| e.amount)
                        .fold(U256::ZERO, |acc, x| acc + x);
                    let net_change = subtract_as_i256(currency_in, currency_out);
                    ...
                    let abs_change = net_change.unsigned_abs();
            ...
            if let Some(token_movs) = self.token_movements.get(addr) {
                for (token_addr, movements) in token_movs {
                    ...
                    if !DENOM_ADDRESSES.contains_key(token_addr) {
                        let token_in: U256 = movements
                            .incoming
                            .values()
                            .map(|e| e.amount)
                            .fold(U256::ZERO, |acc, x| acc + x);
                        let token_out: U256 = movements
                            .outgoing
                            .values()
                            .map(|e| e.amount)
                            .fold(U256::ZERO, |acc, x| acc + x);
                        let net_change = subtract_as_i256(token_in, token_out);
    ...
    fn format_token_movements_struct(&self, addr: &Address) -> HashMap<String, TokenMovement> {
        ...
        if let Some(token_movs) = self.token_movements.get(addr) {
            for (token_addr, movements) in token_movs {
                ...
                result.insert(token_key, TokenMovement { incoming, outgoing });
            }
        }
        ...
        if let Some(currency_movs) = self.currency_movements.get(addr) {
            for (currency_symbol, movements) in currency_movs {
                ...
                result.insert(
                    ...
                    TokenMovement { incoming, outgoing },
                );
            }
        }
